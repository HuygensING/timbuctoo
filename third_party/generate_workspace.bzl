def generated_maven_jars():
  native.maven_jar(
    name = "antlr_antlr",
    artifact = "antlr:antlr:jar:2.7.7",
  )
  native.maven_jar(
    name = "ch_qos_logback_logback_access",
    artifact = "ch.qos.logback:logback-access:jar:1.2.3",
  )
  native.maven_jar(
    name = "ch_qos_logback_logback_classic",
    artifact = "ch.qos.logback:logback-classic:jar:1.2.3",
  )
  native.maven_jar(
    name = "ch_qos_logback_logback_core",
    artifact = "ch.qos.logback:logback-core:jar:1.2.3",
  )
  native.maven_jar(
    name = "com_carrotsearch_hppc",
    artifact = "com.carrotsearch:hppc:jar:0.7.1",
  )
  native.maven_jar(
    name = "com_fasterxml_jackson_core_jackson_annotations",
    artifact = "com.fasterxml.jackson.core:jackson-annotations:jar:2.9.0",
  )
  native.maven_jar(
    name = "com_fasterxml_jackson_core_jackson_core",
    artifact = "com.fasterxml.jackson.core:jackson-core:jar:2.9.1",
  )
  native.maven_jar(
    name = "com_fasterxml_jackson_core_jackson_databind",
    artifact = "com.fasterxml.jackson.core:jackson-databind:jar:2.9.1",
  )
  native.maven_jar(
    name = "com_fasterxml_jackson_dataformat_jackson_dataformat_xml",
    artifact = "com.fasterxml.jackson.dataformat:jackson-dataformat-xml:jar:2.9.1",
  )
  native.maven_jar(
    name = "com_fasterxml_jackson_dataformat_jackson_dataformat_yaml",
    artifact = "com.fasterxml.jackson.dataformat:jackson-dataformat-yaml:jar:2.9.1",
  )
  native.maven_jar(
    name = "com_fasterxml_jackson_datatype_jackson_datatype_guava",
    artifact = "com.fasterxml.jackson.datatype:jackson-datatype-guava:jar:2.9.1",
  )
  native.maven_jar(
    name = "com_fasterxml_jackson_datatype_jackson_datatype_jdk8",
    artifact = "com.fasterxml.jackson.datatype:jackson-datatype-jdk8:jar:2.9.1",
  )
  native.maven_jar(
    name = "com_fasterxml_jackson_datatype_jackson_datatype_joda",
    artifact = "com.fasterxml.jackson.datatype:jackson-datatype-joda:jar:2.9.1",
  )
  native.maven_jar(
    name = "com_fasterxml_jackson_datatype_jackson_datatype_jsr310",
    artifact = "com.fasterxml.jackson.datatype:jackson-datatype-jsr310:jar:2.9.1",
  )
  native.maven_jar(
    name = "com_fasterxml_jackson_jaxrs_jackson_jaxrs_base",
    artifact = "com.fasterxml.jackson.jaxrs:jackson-jaxrs-base:jar:2.9.1",
  )
  native.maven_jar(
    name = "com_fasterxml_jackson_jaxrs_jackson_jaxrs_json_provider",
    artifact = "com.fasterxml.jackson.jaxrs:jackson-jaxrs-json-provider:jar:2.9.1",
  )
  native.maven_jar(
    name = "com_fasterxml_jackson_module_jackson_module_afterburner",
    artifact = "com.fasterxml.jackson.module:jackson-module-afterburner:jar:2.9.1",
  )
  native.maven_jar(
    name = "com_fasterxml_jackson_module_jackson_module_jaxb_annotations",
    artifact = "com.fasterxml.jackson.module:jackson-module-jaxb-annotations:jar:2.9.1",
  )
  native.maven_jar(
    name = "com_fasterxml_jackson_module_jackson_module_parameter_names",
    artifact = "com.fasterxml.jackson.module:jackson-module-parameter-names:jar:2.9.1",
  )
  native.maven_jar(
    name = "com_fasterxml_woodstox_woodstox_core",
    artifact = "com.fasterxml.woodstox:woodstox-core:jar:5.0.3",
  )
  native.maven_jar(
    name = "com_fasterxml_classmate",
    artifact = "com.fasterxml:classmate:jar:1.3.4",
  )
  native.maven_jar(
    name = "com_github_DANS_KNAW_dans_dp_lib",
    artifact = "com.github.DANS-KNAW:dans-dp-lib:jar:36e6037f4b",
    repository = "https://jitpack.io",
  )
  native.maven_jar(
    name = "com_github_andrewoma_dexx_collection",
    artifact = "com.github.andrewoma.dexx:collection:jar:0.6",
  )
  native.maven_jar(
    name = "com_github_fge_btf",
    artifact = "com.github.fge:btf:jar:1.2",
  )
  native.maven_jar(
    name = "com_github_fge_jackson_coreutils",
    artifact = "com.github.fge:jackson-coreutils:jar:1.8",
  )
  native.maven_jar(
    name = "com_github_fge_json_schema_core",
    artifact = "com.github.fge:json-schema-core:jar:1.2.5",
  )
  native.maven_jar(
    name = "com_github_fge_json_schema_validator",
    artifact = "com.github.fge:json-schema-validator:jar:2.2.6",
  )
  native.maven_jar(
    name = "com_github_fge_msg_simple",
    artifact = "com.github.fge:msg-simple:jar:1.1",
  )
  native.maven_jar(
    name = "com_github_fge_uri_template",
    artifact = "com.github.fge:uri-template:jar:0.9",
  )
  native.maven_jar(
    name = "com_github_jsonld_java_jsonld_java",
    artifact = "com.github.jsonld-java:jsonld-java:jar:0.11.0",
  )
  native.maven_jar(
    name = "com_github_virtuald_curvesapi",
    artifact = "com.github.virtuald:curvesapi:jar:1.03",
  )
  native.maven_jar(
    name = "com_google_code_findbugs_jsr305",
    artifact = "com.google.code.findbugs:jsr305:jar:3.0.2",
  )
  native.maven_jar(
    name = "com_google_code_gson_gson",
    artifact = "com.google.code.gson:gson:jar:2.2.4",
  )
  native.maven_jar(
    name = "com_google_guava_guava",
    artifact = "com.google.guava:guava:jar:21.0",
  )
  native.maven_jar(
    name = "com_googlecode_juniversalchardet_juniversalchardet",
    artifact = "com.googlecode.juniversalchardet:juniversalchardet:jar:1.0.3",
  )
  native.maven_jar(
    name = "com_googlecode_libphonenumber_libphonenumber",
    artifact = "com.googlecode.libphonenumber:libphonenumber:jar:6.2",
  )
  native.maven_jar(
    name = "com_graphql_java_graphql_java",
    artifact = "com.graphql-java:graphql-java:jar:5.0",
  )
  native.maven_jar(
    name = "com_graphql_java_java_dataloader",
    artifact = "com.graphql-java:java-dataloader:jar:2.0.1",
  )
  native.maven_jar(
    name = "com_healthmarketscience_jackcess_jackcess",
    artifact = "com.healthmarketscience.jackcess:jackcess:jar:2.1.6",
  )
  native.maven_jar(
    name = "com_javaslang_javaslang",
    artifact = "com.javaslang:javaslang:jar:2.0.0-RC4",
  )
  native.maven_jar(
    name = "com_jcabi_jcabi_log",
    artifact = "com.jcabi:jcabi-log:jar:0.14",
  )
  native.maven_jar(
    name = "com_jcabi_jcabi_manifests",
    artifact = "com.jcabi:jcabi-manifests:jar:1.1",
  )
  native.maven_jar(
    name = "com_jcraft_jzlib",
    artifact = "com.jcraft:jzlib:jar:1.1.3",
  )
  native.maven_jar(
    name = "com_kjetland_dropwizard_dropwizard_activemq",
    artifact = "com.kjetland.dropwizard:dropwizard-activemq:jar:0.3.13",
    repository = "https://raw.githubusercontent.com/mbknor/mbknor.github.com/master/m2repo/releases",
  )
  native.maven_jar(
    name = "com_microsoft_azure_azure_storage",
    artifact = "com.microsoft.azure:azure-storage:jar:4.0.0",
  )
  native.maven_jar(
    name = "com_papertrail_profiler",
    artifact = "com.papertrail:profiler:jar:1.0.2",
  )
  native.maven_jar(
    name = "com_puppycrawl_tools_checkstyle",
    artifact = "com.puppycrawl.tools:checkstyle:jar:6.11.2",
  )
  native.maven_jar(
    name = "com_sleepycat_je",
    artifact = "com.sleepycat:je:jar:7.4.5",
    repository = "http://download.oracle.com/maven",
  )
  native.maven_jar(
    name = "com_twitter_finagle_core_2_11",
    artifact = "com.twitter:finagle-core_2.11:jar:6.33.0",
  )
  native.maven_jar(
    name = "com_twitter_finagle_http_2_11",
    artifact = "com.twitter:finagle-http_2.11:jar:6.33.0",
  )
  native.maven_jar(
    name = "com_twitter_finagle_native_2_11",
    artifact = "com.twitter:finagle-native_2.11:jar:6.33.0",
  )
  native.maven_jar(
    name = "com_twitter_jsr166e",
    artifact = "com.twitter:jsr166e:jar:1.0.0",
  )
  native.maven_jar(
    name = "com_twitter_util_app_2_11",
    artifact = "com.twitter:util-app_2.11:jar:6.32.0",
  )
  native.maven_jar(
    name = "com_twitter_util_cache_2_11",
    artifact = "com.twitter:util-cache_2.11:jar:6.32.0",
  )
  native.maven_jar(
    name = "com_twitter_util_codec_2_11",
    artifact = "com.twitter:util-codec_2.11:jar:6.32.0",
  )
  native.maven_jar(
    name = "com_twitter_util_collection_2_11",
    artifact = "com.twitter:util-collection_2.11:jar:6.32.0",
  )
  native.maven_jar(
    name = "com_twitter_util_core_2_11",
    artifact = "com.twitter:util-core_2.11:jar:6.32.0",
  )
  native.maven_jar(
    name = "com_twitter_util_function_2_11",
    artifact = "com.twitter:util-function_2.11:jar:6.32.0",
  )
  native.maven_jar(
    name = "com_twitter_util_hashing_2_11",
    artifact = "com.twitter:util-hashing_2.11:jar:6.32.0",
  )
  native.maven_jar(
    name = "com_twitter_util_jvm_2_11",
    artifact = "com.twitter:util-jvm_2.11:jar:6.32.0",
  )
  native.maven_jar(
    name = "com_twitter_util_lint_2_11",
    artifact = "com.twitter:util-lint_2.11:jar:6.32.0",
  )
  native.maven_jar(
    name = "com_twitter_util_logging_2_11",
    artifact = "com.twitter:util-logging_2.11:jar:6.32.0",
  )
  native.maven_jar(
    name = "com_twitter_util_registry_2_11",
    artifact = "com.twitter:util-registry_2.11:jar:6.32.0",
  )
  native.maven_jar(
    name = "com_twitter_util_stats_2_11",
    artifact = "com.twitter:util-stats_2.11:jar:6.32.0",
  )
  native.maven_jar(
    name = "commons_beanutils_commons_beanutils",
    artifact = "commons-beanutils:commons-beanutils:jar:1.9.2",
  )
  native.maven_jar(
    name = "commons_cli_commons_cli",
    artifact = "commons-cli:commons-cli:jar:1.3.1",
  )
  native.maven_jar(
    name = "commons_codec_commons_codec",
    artifact = "commons-codec:commons-codec:jar:1.10",
  )
  native.maven_jar(
    name = "commons_collections_commons_collections",
    artifact = "commons-collections:commons-collections:jar:3.2.1",
  )
  native.maven_jar(
    name = "commons_configuration_commons_configuration",
    artifact = "commons-configuration:commons-configuration:jar:1.10",
  )
  native.maven_jar(
    name = "commons_io_commons_io",
    artifact = "commons-io:commons-io:jar:2.4",
  )
  native.maven_jar(
    name = "commons_lang_commons_lang",
    artifact = "commons-lang:commons-lang:jar:2.6",
  )
  native.maven_jar(
    name = "commons_logging_commons_logging",
    artifact = "commons-logging:commons-logging:jar:1.2",
  )
  native.maven_jar(
    name = "commons_net_commons_net",
    artifact = "commons-net:commons-net:jar:3.3",
  )
  native.maven_jar(
    name = "handle_net_handle_client",
    artifact = "handle.net:handle-client:jar:7.3.1",
    repository = "http://maven.huygens.knaw.nl/repository/",
  )
  native.maven_jar(
    name = "io_dropwizard_metrics_metrics_annotation",
    artifact = "io.dropwizard.metrics:metrics-annotation:jar:3.2.5",
  )
  native.maven_jar(
    name = "io_dropwizard_metrics_metrics_core",
    artifact = "io.dropwizard.metrics:metrics-core:jar:3.2.5",
  )
  native.maven_jar(
    name = "io_dropwizard_metrics_metrics_healthchecks",
    artifact = "io.dropwizard.metrics:metrics-healthchecks:jar:3.2.5",
  )
  native.maven_jar(
    name = "io_dropwizard_metrics_metrics_httpclient",
    artifact = "io.dropwizard.metrics:metrics-httpclient:jar:3.2.5",
  )
  native.maven_jar(
    name = "io_dropwizard_metrics_metrics_jersey2",
    artifact = "io.dropwizard.metrics:metrics-jersey2:jar:3.2.5",
  )
  native.maven_jar(
    name = "io_dropwizard_metrics_metrics_jetty9",
    artifact = "io.dropwizard.metrics:metrics-jetty9:jar:3.2.5",
  )
  native.maven_jar(
    name = "io_dropwizard_metrics_metrics_json",
    artifact = "io.dropwizard.metrics:metrics-json:jar:3.2.5",
  )
  native.maven_jar(
    name = "io_dropwizard_metrics_metrics_jvm",
    artifact = "io.dropwizard.metrics:metrics-jvm:jar:3.2.5",
  )
  native.maven_jar(
    name = "io_dropwizard_metrics_metrics_logback",
    artifact = "io.dropwizard.metrics:metrics-logback:jar:3.2.5",
  )
  native.maven_jar(
    name = "io_dropwizard_metrics_metrics_servlets",
    artifact = "io.dropwizard.metrics:metrics-servlets:jar:3.2.5",
  )
  native.maven_jar(
    name = "io_dropwizard_dropwizard_assets",
    artifact = "io.dropwizard:dropwizard-assets:jar:1.2.1",
  )
  native.maven_jar(
    name = "io_dropwizard_dropwizard_client",
    artifact = "io.dropwizard:dropwizard-client:jar:1.2.1",
  )
  native.maven_jar(
    name = "io_dropwizard_dropwizard_configuration",
    artifact = "io.dropwizard:dropwizard-configuration:jar:1.2.1",
  )
  native.maven_jar(
    name = "io_dropwizard_dropwizard_core",
    artifact = "io.dropwizard:dropwizard-core:jar:1.2.1",
  )
  native.maven_jar(
    name = "io_dropwizard_dropwizard_forms",
    artifact = "io.dropwizard:dropwizard-forms:jar:1.2.1",
  )
  native.maven_jar(
    name = "io_dropwizard_dropwizard_jackson",
    artifact = "io.dropwizard:dropwizard-jackson:jar:1.2.1",
  )
  native.maven_jar(
    name = "io_dropwizard_dropwizard_jersey",
    artifact = "io.dropwizard:dropwizard-jersey:jar:1.2.1",
  )
  native.maven_jar(
    name = "io_dropwizard_dropwizard_jetty",
    artifact = "io.dropwizard:dropwizard-jetty:jar:1.2.1",
  )
  native.maven_jar(
    name = "io_dropwizard_dropwizard_lifecycle",
    artifact = "io.dropwizard:dropwizard-lifecycle:jar:1.2.1",
  )
  native.maven_jar(
    name = "io_dropwizard_dropwizard_logging",
    artifact = "io.dropwizard:dropwizard-logging:jar:1.2.1",
  )
  native.maven_jar(
    name = "io_dropwizard_dropwizard_metrics",
    artifact = "io.dropwizard:dropwizard-metrics:jar:1.2.1",
  )
  native.maven_jar(
    name = "io_dropwizard_dropwizard_request_logging",
    artifact = "io.dropwizard:dropwizard-request-logging:jar:1.2.1",
  )
  native.maven_jar(
    name = "io_dropwizard_dropwizard_servlets",
    artifact = "io.dropwizard:dropwizard-servlets:jar:1.2.1",
  )
  native.maven_jar(
    name = "io_dropwizard_dropwizard_testing",
    artifact = "io.dropwizard:dropwizard-testing:jar:1.2.1",
  )
  native.maven_jar(
    name = "io_dropwizard_dropwizard_util",
    artifact = "io.dropwizard:dropwizard-util:jar:1.2.1",
  )
  native.maven_jar(
    name = "io_dropwizard_dropwizard_validation",
    artifact = "io.dropwizard:dropwizard-validation:jar:1.2.1",
  )
  native.maven_jar(
    name = "io_netty_netty",
    artifact = "io.netty:netty:jar:3.9.9.Final",
  )
  native.maven_jar(
    name = "io_netty_netty_buffer",
    artifact = "io.netty:netty-buffer:jar:4.0.34.Final",
  )
  native.maven_jar(
    name = "io_netty_netty_codec",
    artifact = "io.netty:netty-codec:jar:4.0.34.Final",
  )
  native.maven_jar(
    name = "io_netty_netty_codec_http",
    artifact = "io.netty:netty-codec-http:jar:4.0.34.Final",
  )
  native.maven_jar(
    name = "io_netty_netty_codec_socks",
    artifact = "io.netty:netty-codec-socks:jar:4.0.34.Final",
  )
  native.maven_jar(
    name = "io_netty_netty_common",
    artifact = "io.netty:netty-common:jar:4.0.34.Final",
  )
  native.maven_jar(
    name = "io_netty_netty_handler",
    artifact = "io.netty:netty-handler:jar:4.0.34.Final",
  )
  native.maven_jar(
    name = "io_netty_netty_transport",
    artifact = "io.netty:netty-transport:jar:4.0.34.Final",
  )
  native.maven_jar(
    name = "janino_janino",
    artifact = "janino:janino:jar:2.5.10",
  )
  native.maven_jar(
    name = "javax_activation_activation",
    artifact = "javax.activation:activation:jar:1.1",
  )
  native.maven_jar(
    name = "javax_annotation_javax_annotation_api",
    artifact = "javax.annotation:javax.annotation-api:jar:1.2",
  )
  native.maven_jar(
    name = "javax_inject_javax_inject",
    artifact = "javax.inject:javax.inject:jar:1",
  )
  native.maven_jar(
    name = "javax_mail_mailapi",
    artifact = "javax.mail:mailapi:jar:1.4.3",
  )
  native.maven_jar(
    name = "javax_servlet_javax_servlet_api",
    artifact = "javax.servlet:javax.servlet-api:jar:3.1.0",
  )
  native.maven_jar(
    name = "javax_validation_validation_api",
    artifact = "javax.validation:validation-api:jar:1.1.0.Final",
  )
  native.maven_jar(
    name = "javax_ws_rs_javax_ws_rs_api",
    artifact = "javax.ws.rs:javax.ws.rs-api:jar:2.0.1",
  )
  native.maven_jar(
    name = "jline_jline",
    artifact = "jline:jline:jar:2.12",
  )
  native.maven_jar(
    name = "joda_time_joda_time",
    artifact = "joda-time:joda-time:jar:2.9.9",
  )
  native.maven_jar(
    name = "junit_junit",
    artifact = "junit:junit:jar:4.12",
  )
  native.maven_jar(
    name = "me_moocar_logback_gelf",
    artifact = "me.moocar:logback-gelf:jar:0.12",
  )
  native.maven_jar(
    name = "net_bytebuddy_byte_buddy",
    artifact = "net.bytebuddy:byte-buddy:jar:1.5.5",
  )
  native.maven_jar(
    name = "net_bytebuddy_byte_buddy_agent",
    artifact = "net.bytebuddy:byte-buddy-agent:jar:1.5.5",
  )
  native.maven_jar(
    name = "net_gini_dropwizard_dropwizard_gelf",
    artifact = "net.gini.dropwizard:dropwizard-gelf:jar:0.9.2-1",
  )
  native.maven_jar(
    name = "net_sf_jopt_simple_jopt_simple",
    artifact = "net.sf.jopt-simple:jopt-simple:jar:4.6",
  )
  native.maven_jar(
    name = "net_sourceforge_argparse4j_argparse4j",
    artifact = "net.sourceforge.argparse4j:argparse4j:jar:0.7.0",
  )
  native.maven_jar(
    name = "nl_knaw_huygens_huygens_persistence",
    artifact = "nl.knaw.huygens:huygens-persistence:jar:2.2.0",
    repository = "http://maven.huygens.knaw.nl/repository/",
  )
  native.maven_jar(
    name = "nl_knaw_huygens_security_core",
    artifact = "nl.knaw.huygens:security-core:jar:2.2.2.4",
    repository = "http://maven.huygens.knaw.nl/repository/",
  )
  native.maven_jar(
    name = "ognl_ognl",
    artifact = "ognl:ognl:jar:2.6.9",
  )
  native.maven_jar(
    name = "org_antlr_antlr4_runtime",
    artifact = "org.antlr:antlr4-runtime:jar:4.5.1-1",
  )
  native.maven_jar(
    name = "org_apache_activemq_protobuf_activemq_protobuf",
    artifact = "org.apache.activemq.protobuf:activemq-protobuf:jar:1.1",
  )
  native.maven_jar(
    name = "org_apache_activemq_activemq_broker",
    artifact = "org.apache.activemq:activemq-broker:jar:5.13.0",
  )
  native.maven_jar(
    name = "org_apache_activemq_activemq_client",
    artifact = "org.apache.activemq:activemq-client:jar:5.13.0",
  )
  native.maven_jar(
    name = "org_apache_activemq_activemq_jms_pool",
    artifact = "org.apache.activemq:activemq-jms-pool:jar:5.13.0",
  )
  native.maven_jar(
    name = "org_apache_activemq_activemq_kahadb_store",
    artifact = "org.apache.activemq:activemq-kahadb-store:jar:5.13.0",
  )
  native.maven_jar(
    name = "org_apache_activemq_activemq_openwire_legacy",
    artifact = "org.apache.activemq:activemq-openwire-legacy:jar:5.13.0",
  )
  native.maven_jar(
    name = "org_apache_commons_commons_compress",
    artifact = "org.apache.commons:commons-compress:jar:1.12",
  )
  native.maven_jar(
    name = "org_apache_commons_commons_csv",
    artifact = "org.apache.commons:commons-csv:jar:1.4",
  )
  native.maven_jar(
    name = "org_apache_commons_commons_jexl3",
    artifact = "org.apache.commons:commons-jexl3:jar:3.0",
  )
  native.maven_jar(
    name = "org_apache_commons_commons_lang3",
    artifact = "org.apache.commons:commons-lang3:jar:3.4",
  )
  native.maven_jar(
    name = "org_apache_commons_commons_pool2",
    artifact = "org.apache.commons:commons-pool2:jar:2.4.2",
  )
  native.maven_jar(
    name = "org_apache_geronimo_specs_geronimo_j2ee_management_1_1_spec",
    artifact = "org.apache.geronimo.specs:geronimo-j2ee-management_1.1_spec:jar:1.0.1",
  )
  native.maven_jar(
    name = "org_apache_geronimo_specs_geronimo_jms_1_1_spec",
    artifact = "org.apache.geronimo.specs:geronimo-jms_1.1_spec:jar:1.1.1",
  )
  native.maven_jar(
    name = "org_apache_geronimo_specs_geronimo_jta_1_0_1B_spec",
    artifact = "org.apache.geronimo.specs:geronimo-jta_1.0.1B_spec:jar:1.0.1",
  )
  native.maven_jar(
    name = "org_apache_httpcomponents_fluent_hc",
    artifact = "org.apache.httpcomponents:fluent-hc:jar:4.5.3",
  )
  native.maven_jar(
    name = "org_apache_httpcomponents_httpasyncclient",
    artifact = "org.apache.httpcomponents:httpasyncclient:jar:4.1.2",
  )
  native.maven_jar(
    name = "org_apache_httpcomponents_httpclient",
    artifact = "org.apache.httpcomponents:httpclient:jar:4.5.3",
  )
  native.maven_jar(
    name = "org_apache_httpcomponents_httpclient_cache",
    artifact = "org.apache.httpcomponents:httpclient-cache:jar:4.2.6",
  )
  native.maven_jar(
    name = "org_apache_httpcomponents_httpclient_osgi",
    artifact = "org.apache.httpcomponents:httpclient-osgi:jar:4.5.3",
  )
  native.maven_jar(
    name = "org_apache_httpcomponents_httpcore",
    artifact = "org.apache.httpcomponents:httpcore:jar:4.4.5",
  )
  native.maven_jar(
    name = "org_apache_httpcomponents_httpcore_nio",
    artifact = "org.apache.httpcomponents:httpcore-nio:jar:4.4.5",
  )
  native.maven_jar(
    name = "org_apache_httpcomponents_httpcore_osgi",
    artifact = "org.apache.httpcomponents:httpcore-osgi:jar:4.4.6",
  )
  native.maven_jar(
    name = "org_apache_httpcomponents_httpmime",
    artifact = "org.apache.httpcomponents:httpmime:jar:4.5.3",
  )
  native.maven_jar(
    name = "org_apache_ivy_ivy",
    artifact = "org.apache.ivy:ivy:jar:2.3.0",
  )
  native.maven_jar(
    name = "org_apache_jena_jena_arq",
    artifact = "org.apache.jena:jena-arq:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_apache_jena_jena_base",
    artifact = "org.apache.jena:jena-base:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_apache_jena_jena_core",
    artifact = "org.apache.jena:jena-core:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_apache_jena_jena_iri",
    artifact = "org.apache.jena:jena-iri:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_apache_jena_jena_shaded_guava",
    artifact = "org.apache.jena:jena-shaded-guava:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_apache_jena_jena_tdb",
    artifact = "org.apache.jena:jena-tdb:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_apache_lucene_lucene_analyzers_common",
    artifact = "org.apache.lucene:lucene-analyzers-common:jar:5.5.0",
  )
  native.maven_jar(
    name = "org_apache_lucene_lucene_backward_codecs",
    artifact = "org.apache.lucene:lucene-backward-codecs:jar:5.5.0",
  )
  native.maven_jar(
    name = "org_apache_lucene_lucene_codecs",
    artifact = "org.apache.lucene:lucene-codecs:jar:5.5.0",
  )
  native.maven_jar(
    name = "org_apache_lucene_lucene_core",
    artifact = "org.apache.lucene:lucene-core:jar:5.5.0",
  )
  native.maven_jar(
    name = "org_apache_lucene_lucene_queryparser",
    artifact = "org.apache.lucene:lucene-queryparser:jar:5.5.0",
  )
  native.maven_jar(
    name = "org_apache_poi_poi",
    artifact = "org.apache.poi:poi:jar:3.14",
  )
  native.maven_jar(
    name = "org_apache_poi_poi_ooxml",
    artifact = "org.apache.poi:poi-ooxml:jar:3.14",
  )
  native.maven_jar(
    name = "org_apache_poi_poi_ooxml_schemas",
    artifact = "org.apache.poi:poi-ooxml-schemas:jar:3.14",
  )
  native.maven_jar(
    name = "org_apache_thrift_libthrift",
    artifact = "org.apache.thrift:libthrift:jar:0.9.2",
  )
  native.maven_jar(
    name = "org_apache_tinkerpop_gremlin_core",
    artifact = "org.apache.tinkerpop:gremlin-core:jar:3.1.0-incubating",
  )
  native.maven_jar(
    name = "org_apache_tinkerpop_gremlin_groovy",
    artifact = "org.apache.tinkerpop:gremlin-groovy:jar:3.1.0-incubating",
  )
  native.maven_jar(
    name = "org_apache_tinkerpop_gremlin_shaded",
    artifact = "org.apache.tinkerpop:gremlin-shaded:jar:3.1.0-incubating",
  )
  native.maven_jar(
    name = "org_apache_tinkerpop_neo4j_gremlin",
    artifact = "org.apache.tinkerpop:neo4j-gremlin:jar:3.1.0-incubating",
  )
  native.maven_jar(
    name = "org_apache_xmlbeans_xmlbeans",
    artifact = "org.apache.xmlbeans:xmlbeans:jar:2.6.0",
  )
  native.maven_jar(
    name = "org_assertj_assertj_core",
    artifact = "org.assertj:assertj-core:jar:3.8.0",
  )
  native.maven_jar(
    name = "org_bouncycastle_bcmail_jdk15on",
    artifact = "org.bouncycastle:bcmail-jdk15on:jar:1.52",
  )
  native.maven_jar(
    name = "org_bouncycastle_bcpkix_jdk15on",
    artifact = "org.bouncycastle:bcpkix-jdk15on:jar:1.52",
  )
  native.maven_jar(
    name = "org_bouncycastle_bcprov_jdk15on",
    artifact = "org.bouncycastle:bcprov-jdk15on:jar:1.52",
  )
  native.maven_jar(
    name = "org_codehaus_groovy_groovy",
    artifact = "org.codehaus.groovy:groovy:jar:2.4.1",
  )
  native.maven_jar(
    name = "org_codehaus_groovy_groovy_console",
    artifact = "org.codehaus.groovy:groovy-console:jar:2.4.1",
  )
  native.maven_jar(
    name = "org_codehaus_groovy_groovy_groovysh",
    artifact = "org.codehaus.groovy:groovy-groovysh:jar:2.4.1",
  )
  native.maven_jar(
    name = "org_codehaus_groovy_groovy_json",
    artifact = "org.codehaus.groovy:groovy-json:jar:2.4.1",
  )
  native.maven_jar(
    name = "org_codehaus_groovy_groovy_jsr223",
    artifact = "org.codehaus.groovy:groovy-jsr223:jar:2.4.1",
  )
  native.maven_jar(
    name = "org_codehaus_groovy_groovy_swing",
    artifact = "org.codehaus.groovy:groovy-swing:jar:2.4.1",
  )
  native.maven_jar(
    name = "org_codehaus_groovy_groovy_templates",
    artifact = "org.codehaus.groovy:groovy-templates:jar:2.4.1",
  )
  native.maven_jar(
    name = "org_codehaus_groovy_groovy_xml",
    artifact = "org.codehaus.groovy:groovy-xml:jar:2.4.1",
  )
  native.maven_jar(
    name = "org_codehaus_woodstox_stax2_api",
    artifact = "org.codehaus.woodstox:stax2-api:jar:3.1.4",
  )
  native.maven_jar(
    name = "org_concordion_concordion",
    artifact = "org.concordion:concordion:jar:1.5.1",
  )
  native.maven_jar(
    name = "org_eclipse_jetty_toolchain_setuid_jetty_setuid_java",
    artifact = "org.eclipse.jetty.toolchain.setuid:jetty-setuid-java:jar:1.0.3",
  )
  native.maven_jar(
    name = "org_eclipse_jetty_jetty_continuation",
    artifact = "org.eclipse.jetty:jetty-continuation:jar:9.4.7.v20170914",
  )
  native.maven_jar(
    name = "org_eclipse_jetty_jetty_http",
    artifact = "org.eclipse.jetty:jetty-http:jar:9.4.7.v20170914",
  )
  native.maven_jar(
    name = "org_eclipse_jetty_jetty_io",
    artifact = "org.eclipse.jetty:jetty-io:jar:9.4.7.v20170914",
  )
  native.maven_jar(
    name = "org_eclipse_jetty_jetty_security",
    artifact = "org.eclipse.jetty:jetty-security:jar:9.4.7.v20170914",
  )
  native.maven_jar(
    name = "org_eclipse_jetty_jetty_server",
    artifact = "org.eclipse.jetty:jetty-server:jar:9.4.7.v20170914",
  )
  native.maven_jar(
    name = "org_eclipse_jetty_jetty_servlet",
    artifact = "org.eclipse.jetty:jetty-servlet:jar:9.4.7.v20170914",
  )
  native.maven_jar(
    name = "org_eclipse_jetty_jetty_servlets",
    artifact = "org.eclipse.jetty:jetty-servlets:jar:9.4.7.v20170914",
  )
  native.maven_jar(
    name = "org_eclipse_jetty_jetty_util",
    artifact = "org.eclipse.jetty:jetty-util:jar:9.4.7.v20170914",
  )
  native.maven_jar(
    name = "org_eclipse_jetty_jetty_webapp",
    artifact = "org.eclipse.jetty:jetty-webapp:jar:9.4.7.v20170914",
  )
  native.maven_jar(
    name = "org_eclipse_jetty_jetty_xml",
    artifact = "org.eclipse.jetty:jetty-xml:jar:9.4.7.v20170914",
  )
  native.maven_jar(
    name = "org_eclipse_rdf4j_rdf4j_model",
    artifact = "org.eclipse.rdf4j:rdf4j-model:jar:2.2",
  )
  native.maven_jar(
    name = "org_eclipse_rdf4j_rdf4j_rio_api",
    artifact = "org.eclipse.rdf4j:rdf4j-rio-api:jar:2.2",
  )
  native.maven_jar(
    name = "org_eclipse_rdf4j_rdf4j_rio_datatypes",
    artifact = "org.eclipse.rdf4j:rdf4j-rio-datatypes:jar:2.2",
  )
  native.maven_jar(
    name = "org_eclipse_rdf4j_rdf4j_rio_jsonld",
    artifact = "org.eclipse.rdf4j:rdf4j-rio-jsonld:jar:2.2",
  )
  native.maven_jar(
    name = "org_eclipse_rdf4j_rdf4j_rio_languages",
    artifact = "org.eclipse.rdf4j:rdf4j-rio-languages:jar:2.2",
  )
  native.maven_jar(
    name = "org_eclipse_rdf4j_rdf4j_rio_n3",
    artifact = "org.eclipse.rdf4j:rdf4j-rio-n3:jar:2.2",
  )
  native.maven_jar(
    name = "org_eclipse_rdf4j_rdf4j_rio_nquads",
    artifact = "org.eclipse.rdf4j:rdf4j-rio-nquads:jar:2.2",
  )
  native.maven_jar(
    name = "org_eclipse_rdf4j_rdf4j_rio_ntriples",
    artifact = "org.eclipse.rdf4j:rdf4j-rio-ntriples:jar:2.2",
  )
  native.maven_jar(
    name = "org_eclipse_rdf4j_rdf4j_rio_rdfxml",
    artifact = "org.eclipse.rdf4j:rdf4j-rio-rdfxml:jar:2.2",
  )
  native.maven_jar(
    name = "org_eclipse_rdf4j_rdf4j_rio_trig",
    artifact = "org.eclipse.rdf4j:rdf4j-rio-trig:jar:2.2",
  )
  native.maven_jar(
    name = "org_eclipse_rdf4j_rdf4j_rio_turtle",
    artifact = "org.eclipse.rdf4j:rdf4j-rio-turtle:jar:2.2",
  )
  native.maven_jar(
    name = "org_eclipse_rdf4j_rdf4j_util",
    artifact = "org.eclipse.rdf4j:rdf4j-util:jar:2.2",
  )
  native.maven_jar(
    name = "org_elasticsearch_client_rest",
    artifact = "org.elasticsearch.client:rest:jar:5.5.3",
  )
  native.maven_jar(
    name = "org_fusesource_hawtbuf_hawtbuf",
    artifact = "org.fusesource.hawtbuf:hawtbuf:jar:1.11",
  )
  native.maven_jar(
    name = "org_glassfish_hk2_external_aopalliance_repackaged",
    artifact = "org.glassfish.hk2.external:aopalliance-repackaged:jar:2.5.0-b32",
  )
  native.maven_jar(
    name = "org_glassfish_hk2_external_javax_inject",
    artifact = "org.glassfish.hk2.external:javax.inject:jar:2.5.0-b32",
  )
  native.maven_jar(
    name = "org_glassfish_hk2_hk2_api",
    artifact = "org.glassfish.hk2:hk2-api:jar:2.5.0-b32",
  )
  native.maven_jar(
    name = "org_glassfish_hk2_hk2_locator",
    artifact = "org.glassfish.hk2:hk2-locator:jar:2.5.0-b32",
  )
  native.maven_jar(
    name = "org_glassfish_hk2_hk2_utils",
    artifact = "org.glassfish.hk2:hk2-utils:jar:2.5.0-b32",
  )
  native.maven_jar(
    name = "org_glassfish_hk2_osgi_resource_locator",
    artifact = "org.glassfish.hk2:osgi-resource-locator:jar:1.0.1",
  )
  native.maven_jar(
    name = "org_glassfish_jaxb_txw2",
    artifact = "org.glassfish.jaxb:txw2:jar:2.2.11",
  )
  native.maven_jar(
    name = "org_glassfish_jersey_bundles_repackaged_jersey_guava",
    artifact = "org.glassfish.jersey.bundles.repackaged:jersey-guava:jar:2.25.1",
  )
  native.maven_jar(
    name = "org_glassfish_jersey_connectors_jersey_apache_connector",
    artifact = "org.glassfish.jersey.connectors:jersey-apache-connector:jar:2.25.1",
  )
  native.maven_jar(
    name = "org_glassfish_jersey_containers_jersey_container_servlet",
    artifact = "org.glassfish.jersey.containers:jersey-container-servlet:jar:2.25.1",
  )
  native.maven_jar(
    name = "org_glassfish_jersey_containers_jersey_container_servlet_core",
    artifact = "org.glassfish.jersey.containers:jersey-container-servlet-core:jar:2.25.1",
  )
  native.maven_jar(
    name = "org_glassfish_jersey_core_jersey_client",
    artifact = "org.glassfish.jersey.core:jersey-client:jar:2.25.1",
  )
  native.maven_jar(
    name = "org_glassfish_jersey_core_jersey_common",
    artifact = "org.glassfish.jersey.core:jersey-common:jar:2.25.1",
  )
  native.maven_jar(
    name = "org_glassfish_jersey_core_jersey_server",
    artifact = "org.glassfish.jersey.core:jersey-server:jar:2.25.1",
  )
  native.maven_jar(
    name = "org_glassfish_jersey_ext_rx_jersey_rx_client",
    artifact = "org.glassfish.jersey.ext.rx:jersey-rx-client:jar:2.25.1",
  )
  native.maven_jar(
    name = "org_glassfish_jersey_ext_jersey_bean_validation",
    artifact = "org.glassfish.jersey.ext:jersey-bean-validation:jar:2.25.1",
  )
  native.maven_jar(
    name = "org_glassfish_jersey_ext_jersey_metainf_services",
    artifact = "org.glassfish.jersey.ext:jersey-metainf-services:jar:2.25.1",
  )
  native.maven_jar(
    name = "org_glassfish_jersey_media_jersey_media_jaxb",
    artifact = "org.glassfish.jersey.media:jersey-media-jaxb:jar:2.25.1",
  )
  native.maven_jar(
    name = "org_glassfish_jersey_media_jersey_media_multipart",
    artifact = "org.glassfish.jersey.media:jersey-media-multipart:jar:2.25.1",
  )
  native.maven_jar(
    name = "org_glassfish_jersey_test_framework_providers_jersey_test_framework_provider_inmemory",
    artifact = "org.glassfish.jersey.test-framework.providers:jersey-test-framework-provider-inmemory:jar:2.25.1",
  )
  native.maven_jar(
    name = "org_glassfish_jersey_test_framework_jersey_test_framework_core",
    artifact = "org.glassfish.jersey.test-framework:jersey-test-framework-core:jar:2.25.1",
  )
  native.maven_jar(
    name = "org_glassfish_javax_el",
    artifact = "org.glassfish:javax.el:jar:3.0.0",
  )
  native.maven_jar(
    name = "org_hamcrest_hamcrest_all",
    artifact = "org.hamcrest:hamcrest-all:jar:1.3",
  )
  native.maven_jar(
    name = "org_hamcrest_hamcrest_core",
    artifact = "org.hamcrest:hamcrest-core:jar:1.3",
  )
  native.maven_jar(
    name = "org_hibernate_hibernate_validator",
    artifact = "org.hibernate:hibernate-validator:jar:5.4.1.Final",
  )
  native.maven_jar(
    name = "org_immutables_value",
    artifact = "org.immutables:value:jar:2.5.0",
  )
  native.maven_jar(
    name = "org_javassist_javassist",
    artifact = "org.javassist:javassist:jar:3.21.0-GA",
  )
  native.maven_jar(
    name = "org_javatuples_javatuples",
    artifact = "org.javatuples:javatuples:jar:1.2",
  )
  native.maven_jar(
    name = "org_jboss_logging_jboss_logging",
    artifact = "org.jboss.logging:jboss-logging:jar:3.3.0.Final",
  )
  native.maven_jar(
    name = "org_json_json",
    artifact = "org.json:json:jar:20090211",
  )
  native.maven_jar(
    name = "org_jsoup_jsoup",
    artifact = "org.jsoup:jsoup:jar:1.9.2",
  )
  native.maven_jar(
    name = "org_jvnet_mimepull_mimepull",
    artifact = "org.jvnet.mimepull:mimepull:jar:1.9.6",
  )
  native.maven_jar(
    name = "org_mindrot_jbcrypt",
    artifact = "org.mindrot:jbcrypt:jar:0.3m",
  )
  native.maven_jar(
    name = "org_mock_server_mockserver_client_java",
    artifact = "org.mock-server:mockserver-client-java:jar:3.10.4",
  )
  native.maven_jar(
    name = "org_mock_server_mockserver_core",
    artifact = "org.mock-server:mockserver-core:jar:3.10.4",
  )
  native.maven_jar(
    name = "org_mock_server_mockserver_logging",
    artifact = "org.mock-server:mockserver-logging:jar:3.10.4",
  )
  native.maven_jar(
    name = "org_mock_server_mockserver_netty",
    artifact = "org.mock-server:mockserver-netty:jar:3.10.4",
  )
  native.maven_jar(
    name = "org_mockito_mockito_core",
    artifact = "org.mockito:mockito-core:jar:2.2.22",
  )
  native.maven_jar(
    name = "org_mozilla_rhino",
    artifact = "org.mozilla:rhino:jar:1.7R4",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_backup",
    artifact = "org.neo4j:neo4j-backup:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_cluster",
    artifact = "org.neo4j:neo4j-cluster:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_collections",
    artifact = "org.neo4j:neo4j-collections:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_com",
    artifact = "org.neo4j:neo4j-com:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_command_line",
    artifact = "org.neo4j:neo4j-command-line:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_common",
    artifact = "org.neo4j:neo4j-common:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_consistency_check",
    artifact = "org.neo4j:neo4j-consistency-check:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_csv",
    artifact = "org.neo4j:neo4j-csv:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_dbms",
    artifact = "org.neo4j:neo4j-dbms:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_enterprise_kernel",
    artifact = "org.neo4j:neo4j-enterprise-kernel:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_graphdb_api",
    artifact = "org.neo4j:neo4j-graphdb-api:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_ha",
    artifact = "org.neo4j:neo4j-ha:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_import_tool",
    artifact = "org.neo4j:neo4j-import-tool:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_io",
    artifact = "org.neo4j:neo4j-io:jar:3.1.0",
  )
  native.http_jar(
    name = "org_neo4j_neo4j_io_test_jar",
    url = "http://repo1.maven.org/maven2/org/neo4j/neo4j-io/3.1.0/neo4j-io-3.1.0-tests.jar",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_kernel",
    artifact = "org.neo4j:neo4j-kernel:jar:3.1.0",
  )
  native.http_jar(
    name = "org_neo4j_neo4j_kernel_test_jar",
    url = "http://repo1.maven.org/maven2/org/neo4j/neo4j-kernel/3.1.0/neo4j-kernel-3.1.0-tests.jar",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_logging",
    artifact = "org.neo4j:neo4j-logging:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_lucene_index",
    artifact = "org.neo4j:neo4j-lucene-index:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_lucene_upgrade",
    artifact = "org.neo4j:neo4j-lucene-upgrade:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_primitive_collections",
    artifact = "org.neo4j:neo4j-primitive-collections:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_resource",
    artifact = "org.neo4j:neo4j-resource:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_slf4j",
    artifact = "org.neo4j:neo4j-slf4j:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_tinkerpop_api",
    artifact = "org.neo4j:neo4j-tinkerpop-api:jar:0.1",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_tinkerpop_api_impl",
    artifact = "org.neo4j:neo4j-tinkerpop-api-impl:jar:0.4-3.0.3",
  )
  native.maven_jar(
    name = "org_neo4j_neo4j_unsafe",
    artifact = "org.neo4j:neo4j-unsafe:jar:3.1.0",
  )
  native.maven_jar(
    name = "org_objenesis_objenesis",
    artifact = "org.objenesis:objenesis:jar:2.6",
  )
  native.maven_jar(
    name = "org_scala_lang_modules_scala_parser_combinators_2_11",
    artifact = "org.scala-lang.modules:scala-parser-combinators_2.11:jar:1.0.4",
  )
  native.maven_jar(
    name = "org_scala_lang_scala_library",
    artifact = "org.scala-lang:scala-library:jar:2.11.7",
  )
  native.maven_jar(
    name = "org_skyscreamer_jsonassert",
    artifact = "org.skyscreamer:jsonassert:jar:1.1.1",
  )
  native.maven_jar(
    name = "org_slf4j_jcl_over_slf4j",
    artifact = "org.slf4j:jcl-over-slf4j:jar:1.7.25",
  )
  native.maven_jar(
    name = "org_slf4j_jul_to_slf4j",
    artifact = "org.slf4j:jul-to-slf4j:jar:1.7.25",
  )
  native.maven_jar(
    name = "org_slf4j_log4j_over_slf4j",
    artifact = "org.slf4j:log4j-over-slf4j:jar:1.7.25",
  )
  native.maven_jar(
    name = "org_slf4j_slf4j_api",
    artifact = "org.slf4j:slf4j-api:jar:1.7.25",
  )
  native.maven_jar(
    name = "org_twitter4j_twitter4j_core",
    artifact = "org.twitter4j:twitter4j-core:jar:4.0.6",
  )
  native.maven_jar(
    name = "org_xmlunit_xmlunit_core",
    artifact = "org.xmlunit:xmlunit-core:jar:2.4.0",
  )
  native.maven_jar(
    name = "org_xmlunit_xmlunit_matchers",
    artifact = "org.xmlunit:xmlunit-matchers:jar:2.4.0",
  )
  native.maven_jar(
    name = "org_yaml_snakeyaml",
    artifact = "org.yaml:snakeyaml:jar:1.15",
  )
  native.maven_jar(
    name = "stax_stax_api",
    artifact = "stax:stax-api:jar:1.0.1",
  )
  native.maven_jar(
    name = "uk_co_datumedge_hamcrest_json",
    artifact = "uk.co.datumedge:hamcrest-json:jar:0.2",
  )
  native.maven_jar(
    name = "xerces_xercesImpl",
    artifact = "xerces:xercesImpl:jar:2.11.0",
  )
  native.maven_jar(
    name = "xml_apis_xml_apis",
    artifact = "xml-apis:xml-apis:jar:1.4.01",
  )
  native.maven_jar(
    name = "xom_xom",
    artifact = "xom:xom:jar:1.2.5",
  )
def generated_java_libraries():
  native.java_library(
    name = "antlr_antlr",
    visibility = ["//visibility:public"],
    exports = ["@antlr_antlr//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "ch_qos_logback_logback_access",
    visibility = ["//visibility:public"],
    exports = ["@ch_qos_logback_logback_access//jar"],
    runtime_deps = [
      ":ch_qos_logback_logback_core",
    ],
  )
  native.java_library(
    name = "ch_qos_logback_logback_classic",
    visibility = ["//visibility:public"],
    exports = ["@ch_qos_logback_logback_classic//jar"],
    runtime_deps = [
      ":ch_qos_logback_logback_core",
    ],
  )
  native.java_library(
    name = "ch_qos_logback_logback_core",
    visibility = ["//visibility:public"],
    exports = ["@ch_qos_logback_logback_core//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "com_carrotsearch_hppc",
    visibility = ["//visibility:public"],
    exports = ["@com_carrotsearch_hppc//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "com_fasterxml_jackson_core_jackson_annotations",
    visibility = ["//visibility:public"],
    exports = ["@com_fasterxml_jackson_core_jackson_annotations//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "com_fasterxml_jackson_core_jackson_core",
    visibility = ["//visibility:public"],
    exports = ["@com_fasterxml_jackson_core_jackson_core//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "com_fasterxml_jackson_core_jackson_databind",
    visibility = ["//visibility:public"],
    exports = ["@com_fasterxml_jackson_core_jackson_databind//jar"],
    runtime_deps = [
      ":com_fasterxml_jackson_core_jackson_annotations",
      ":com_fasterxml_jackson_core_jackson_core",
    ],
  )
  native.java_library(
    name = "com_fasterxml_jackson_dataformat_jackson_dataformat_xml",
    visibility = ["//visibility:public"],
    exports = ["@com_fasterxml_jackson_dataformat_jackson_dataformat_xml//jar"],
    runtime_deps = [
      ":com_fasterxml_jackson_core_jackson_core",
      ":com_fasterxml_jackson_core_jackson_annotations",
      ":com_fasterxml_jackson_core_jackson_databind",
      ":com_fasterxml_jackson_module_jackson_module_jaxb_annotations",
      ":org_codehaus_woodstox_stax2_api",
      ":com_fasterxml_woodstox_woodstox_core",
    ],
  )
  native.java_library(
    name = "com_fasterxml_jackson_dataformat_jackson_dataformat_yaml",
    visibility = ["//visibility:public"],
    exports = ["@com_fasterxml_jackson_dataformat_jackson_dataformat_yaml//jar"],
    runtime_deps = [
      ":org_yaml_snakeyaml",
      ":com_fasterxml_jackson_core_jackson_core",
    ],
  )
  native.java_library(
    name = "com_fasterxml_jackson_datatype_jackson_datatype_guava",
    visibility = ["//visibility:public"],
    exports = ["@com_fasterxml_jackson_datatype_jackson_datatype_guava//jar"],
    runtime_deps = [
      ":com_google_guava_guava",
      ":com_fasterxml_jackson_core_jackson_core",
      ":com_fasterxml_jackson_core_jackson_databind",
    ],
  )
  native.java_library(
    name = "com_fasterxml_jackson_datatype_jackson_datatype_jdk8",
    visibility = ["//visibility:public"],
    exports = ["@com_fasterxml_jackson_datatype_jackson_datatype_jdk8//jar"],
    runtime_deps = [
      ":com_fasterxml_jackson_core_jackson_core",
      ":com_fasterxml_jackson_core_jackson_databind",
    ],
  )
  native.java_library(
    name = "com_fasterxml_jackson_datatype_jackson_datatype_joda",
    visibility = ["//visibility:public"],
    exports = ["@com_fasterxml_jackson_datatype_jackson_datatype_joda//jar"],
    runtime_deps = [
      ":com_fasterxml_jackson_core_jackson_annotations",
      ":com_fasterxml_jackson_core_jackson_core",
      ":com_fasterxml_jackson_core_jackson_databind",
      ":joda_time_joda_time",
    ],
  )
  native.java_library(
    name = "com_fasterxml_jackson_datatype_jackson_datatype_jsr310",
    visibility = ["//visibility:public"],
    exports = ["@com_fasterxml_jackson_datatype_jackson_datatype_jsr310//jar"],
    runtime_deps = [
      ":com_fasterxml_jackson_core_jackson_annotations",
      ":com_fasterxml_jackson_core_jackson_core",
      ":com_fasterxml_jackson_core_jackson_databind",
    ],
  )
  native.java_library(
    name = "com_fasterxml_jackson_jaxrs_jackson_jaxrs_base",
    visibility = ["//visibility:public"],
    exports = ["@com_fasterxml_jackson_jaxrs_jackson_jaxrs_base//jar"],
    runtime_deps = [
      ":com_fasterxml_jackson_core_jackson_core",
      ":com_fasterxml_jackson_core_jackson_databind",
    ],
  )
  native.java_library(
    name = "com_fasterxml_jackson_jaxrs_jackson_jaxrs_json_provider",
    visibility = ["//visibility:public"],
    exports = ["@com_fasterxml_jackson_jaxrs_jackson_jaxrs_json_provider//jar"],
    runtime_deps = [
      ":com_fasterxml_jackson_jaxrs_jackson_jaxrs_base",
      ":com_fasterxml_jackson_module_jackson_module_jaxb_annotations",
    ],
  )
  native.java_library(
    name = "com_fasterxml_jackson_module_jackson_module_afterburner",
    visibility = ["//visibility:public"],
    exports = ["@com_fasterxml_jackson_module_jackson_module_afterburner//jar"],
    runtime_deps = [
      ":com_fasterxml_jackson_core_jackson_core",
      ":com_fasterxml_jackson_core_jackson_databind",
    ],
  )
  native.java_library(
    name = "com_fasterxml_jackson_module_jackson_module_jaxb_annotations",
    visibility = ["//visibility:public"],
    exports = ["@com_fasterxml_jackson_module_jackson_module_jaxb_annotations//jar"],
    runtime_deps = [
      ":com_fasterxml_jackson_core_jackson_annotations",
      ":com_fasterxml_jackson_core_jackson_core",
      ":com_fasterxml_jackson_core_jackson_databind",
    ],
  )
  native.java_library(
    name = "com_fasterxml_jackson_module_jackson_module_parameter_names",
    visibility = ["//visibility:public"],
    exports = ["@com_fasterxml_jackson_module_jackson_module_parameter_names//jar"],
    runtime_deps = [
      ":com_fasterxml_jackson_core_jackson_core",
      ":com_fasterxml_jackson_core_jackson_databind",
    ],
  )
  native.java_library(
    name = "com_fasterxml_woodstox_woodstox_core",
    visibility = ["//visibility:public"],
    exports = ["@com_fasterxml_woodstox_woodstox_core//jar"],
    runtime_deps = [
      ":org_codehaus_woodstox_stax2_api",
    ],
  )
  native.java_library(
    name = "com_fasterxml_classmate",
    visibility = ["//visibility:public"],
    exports = ["@com_fasterxml_classmate//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "com_github_DANS_KNAW_dans_dp_lib",
    visibility = ["//visibility:public"],
    exports = ["@com_github_DANS_KNAW_dans_dp_lib//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "com_github_andrewoma_dexx_collection",
    visibility = ["//visibility:public"],
    exports = ["@com_github_andrewoma_dexx_collection//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "com_github_fge_btf",
    visibility = ["//visibility:public"],
    exports = ["@com_github_fge_btf//jar"],
    runtime_deps = [
      ":com_google_code_findbugs_jsr305",
    ],
  )
  native.java_library(
    name = "com_github_fge_jackson_coreutils",
    visibility = ["//visibility:public"],
    exports = ["@com_github_fge_jackson_coreutils//jar"],
    runtime_deps = [
      ":com_github_fge_msg_simple",
      ":com_fasterxml_jackson_core_jackson_databind",
      ":com_google_guava_guava",
      ":com_google_code_findbugs_jsr305",
    ],
  )
  native.java_library(
    name = "com_github_fge_json_schema_core",
    visibility = ["//visibility:public"],
    exports = ["@com_github_fge_json_schema_core//jar"],
    runtime_deps = [
      ":com_github_fge_uri_template",
      ":com_github_fge_jackson_coreutils",
      ":org_mozilla_rhino",
      ":com_google_code_findbugs_jsr305",
    ],
  )
  native.java_library(
    name = "com_github_fge_json_schema_validator",
    visibility = ["//visibility:public"],
    exports = ["@com_github_fge_json_schema_validator//jar"],
    runtime_deps = [
      ":com_google_code_findbugs_jsr305",
      ":joda_time_joda_time",
      ":com_googlecode_libphonenumber_libphonenumber",
      ":com_github_fge_json_schema_core",
      ":javax_mail_mailapi",
      ":net_sf_jopt_simple_jopt_simple",
    ],
  )
  native.java_library(
    name = "com_github_fge_msg_simple",
    visibility = ["//visibility:public"],
    exports = ["@com_github_fge_msg_simple//jar"],
    runtime_deps = [
      ":com_github_fge_btf",
      ":com_google_code_findbugs_jsr305",
    ],
  )
  native.java_library(
    name = "com_github_fge_uri_template",
    visibility = ["//visibility:public"],
    exports = ["@com_github_fge_uri_template//jar"],
    runtime_deps = [
      ":com_github_fge_msg_simple",
      ":com_google_guava_guava",
      ":com_google_code_findbugs_jsr305",
    ],
  )
  native.java_library(
    name = "com_github_jsonld_java_jsonld_java",
    visibility = ["//visibility:public"],
    exports = ["@com_github_jsonld_java_jsonld_java//jar"],
    runtime_deps = [
      ":com_fasterxml_jackson_core_jackson_core",
      ":com_fasterxml_jackson_core_jackson_databind",
      ":org_apache_httpcomponents_httpclient_osgi",
      ":org_apache_httpcomponents_httpcore_osgi",
      ":org_slf4j_slf4j_api",
      ":org_slf4j_jcl_over_slf4j",
      ":commons_io_commons_io",
    ],
  )
  native.java_library(
    name = "com_github_virtuald_curvesapi",
    visibility = ["//visibility:public"],
    exports = ["@com_github_virtuald_curvesapi//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "com_google_code_findbugs_jsr305",
    visibility = ["//visibility:public"],
    exports = ["@com_google_code_findbugs_jsr305//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "com_google_code_gson_gson",
    visibility = ["//visibility:public"],
    exports = ["@com_google_code_gson_gson//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "com_google_guava_guava",
    visibility = ["//visibility:public"],
    exports = ["@com_google_guava_guava//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "com_googlecode_juniversalchardet_juniversalchardet",
    visibility = ["//visibility:public"],
    exports = ["@com_googlecode_juniversalchardet_juniversalchardet//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "com_googlecode_libphonenumber_libphonenumber",
    visibility = ["//visibility:public"],
    exports = ["@com_googlecode_libphonenumber_libphonenumber//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "com_graphql_java_graphql_java",
    visibility = ["//visibility:public"],
    exports = ["@com_graphql_java_graphql_java//jar"],
    runtime_deps = [
      ":org_antlr_antlr4_runtime",
      ":org_slf4j_slf4j_api",
      ":com_graphql_java_java_dataloader",
    ],
  )
  native.java_library(
    name = "com_graphql_java_java_dataloader",
    visibility = ["//visibility:public"],
    exports = ["@com_graphql_java_java_dataloader//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "com_healthmarketscience_jackcess_jackcess",
    visibility = ["//visibility:public"],
    exports = ["@com_healthmarketscience_jackcess_jackcess//jar"],
    runtime_deps = [
      ":commons_lang_commons_lang",
      ":commons_logging_commons_logging",
    ],
  )
  native.java_library(
    name = "com_javaslang_javaslang",
    visibility = ["//visibility:public"],
    exports = ["@com_javaslang_javaslang//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "com_jcabi_jcabi_log",
    visibility = ["//visibility:public"],
    exports = ["@com_jcabi_jcabi_log//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "com_jcabi_jcabi_manifests",
    visibility = ["//visibility:public"],
    exports = ["@com_jcabi_jcabi_manifests//jar"],
    runtime_deps = [
      ":com_jcabi_jcabi_log",
    ],
  )
  native.java_library(
    name = "com_jcraft_jzlib",
    visibility = ["//visibility:public"],
    exports = ["@com_jcraft_jzlib//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "com_kjetland_dropwizard_dropwizard_activemq",
    visibility = ["//visibility:public"],
    exports = ["@com_kjetland_dropwizard_dropwizard_activemq//jar"],
    runtime_deps = [
      ":io_dropwizard_dropwizard_core",
      ":org_apache_activemq_activemq_client",
      ":org_apache_activemq_activemq_jms_pool",
    ],
  )
  native.java_library(
    name = "com_microsoft_azure_azure_storage",
    visibility = ["//visibility:public"],
    exports = ["@com_microsoft_azure_azure_storage//jar"],
    runtime_deps = [
      ":com_fasterxml_jackson_core_jackson_core",
      ":org_slf4j_slf4j_api",
      ":org_apache_commons_commons_lang3",
    ],
  )
  native.java_library(
    name = "com_papertrail_profiler",
    visibility = ["//visibility:public"],
    exports = ["@com_papertrail_profiler//jar"],
    runtime_deps = [
      ":joda_time_joda_time",
    ],
  )
  native.java_library(
    name = "com_puppycrawl_tools_checkstyle",
    visibility = ["//visibility:public"],
    exports = ["@com_puppycrawl_tools_checkstyle//jar"],
    runtime_deps = [
      ":org_apache_commons_commons_lang3",
      ":antlr_antlr",
      ":org_antlr_antlr4_runtime",
      ":commons_beanutils_commons_beanutils",
      ":commons_cli_commons_cli",
      ":com_google_guava_guava",
    ],
  )
  native.java_library(
    name = "com_sleepycat_je",
    visibility = ["//visibility:public"],
    exports = ["@com_sleepycat_je//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "com_twitter_finagle_core_2_11",
    visibility = ["//visibility:public"],
    exports = ["@com_twitter_finagle_core_2_11//jar"],
    runtime_deps = [
      ":org_scala_lang_scala_library",
      ":com_twitter_util_app_2_11",
      ":com_twitter_util_cache_2_11",
      ":com_twitter_util_codec_2_11",
      ":com_twitter_util_collection_2_11",
      ":com_twitter_util_core_2_11",
      ":com_twitter_util_hashing_2_11",
      ":com_twitter_util_jvm_2_11",
      ":com_twitter_util_lint_2_11",
      ":com_twitter_util_logging_2_11",
      ":com_twitter_util_registry_2_11",
      ":com_twitter_util_stats_2_11",
      ":commons_codec_commons_codec",
      ":com_google_guava_guava",
      ":com_twitter_jsr166e",
      ":io_netty_netty",
    ],
  )
  native.java_library(
    name = "com_twitter_finagle_http_2_11",
    visibility = ["//visibility:public"],
    exports = ["@com_twitter_finagle_http_2_11//jar"],
    runtime_deps = [
      ":org_scala_lang_scala_library",
      ":com_twitter_finagle_core_2_11",
      ":com_twitter_util_codec_2_11",
      ":com_twitter_util_logging_2_11",
      ":commons_lang_commons_lang",
      ":com_google_guava_guava",
    ],
  )
  native.java_library(
    name = "com_twitter_finagle_native_2_11",
    visibility = ["//visibility:public"],
    exports = ["@com_twitter_finagle_native_2_11//jar"],
    runtime_deps = [
      ":org_scala_lang_scala_library",
      ":com_twitter_finagle_core_2_11",
      ":com_twitter_finagle_http_2_11",
    ],
  )
  native.java_library(
    name = "com_twitter_jsr166e",
    visibility = ["//visibility:public"],
    exports = ["@com_twitter_jsr166e//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "com_twitter_util_app_2_11",
    visibility = ["//visibility:public"],
    exports = ["@com_twitter_util_app_2_11//jar"],
    runtime_deps = [
      ":org_scala_lang_scala_library",
      ":com_twitter_util_core_2_11",
      ":com_twitter_util_registry_2_11",
    ],
  )
  native.java_library(
    name = "com_twitter_util_cache_2_11",
    visibility = ["//visibility:public"],
    exports = ["@com_twitter_util_cache_2_11//jar"],
    runtime_deps = [
      ":org_scala_lang_scala_library",
      ":com_twitter_util_core_2_11",
      ":com_google_code_findbugs_jsr305",
      ":com_google_guava_guava",
    ],
  )
  native.java_library(
    name = "com_twitter_util_codec_2_11",
    visibility = ["//visibility:public"],
    exports = ["@com_twitter_util_codec_2_11//jar"],
    runtime_deps = [
      ":org_scala_lang_scala_library",
      ":com_twitter_util_core_2_11",
      ":commons_codec_commons_codec",
    ],
  )
  native.java_library(
    name = "com_twitter_util_collection_2_11",
    visibility = ["//visibility:public"],
    exports = ["@com_twitter_util_collection_2_11//jar"],
    runtime_deps = [
      ":org_scala_lang_scala_library",
      ":com_twitter_util_core_2_11",
      ":com_google_code_findbugs_jsr305",
      ":javax_inject_javax_inject",
      ":com_google_guava_guava",
      ":commons_collections_commons_collections",
    ],
  )
  native.java_library(
    name = "com_twitter_util_core_2_11",
    visibility = ["//visibility:public"],
    exports = ["@com_twitter_util_core_2_11//jar"],
    runtime_deps = [
      ":org_scala_lang_scala_library",
      ":com_twitter_util_function_2_11",
      ":com_twitter_jsr166e",
      ":org_scala_lang_modules_scala_parser_combinators_2_11",
    ],
  )
  native.java_library(
    name = "com_twitter_util_function_2_11",
    visibility = ["//visibility:public"],
    exports = ["@com_twitter_util_function_2_11//jar"],
    runtime_deps = [
      ":org_scala_lang_scala_library",
    ],
  )
  native.java_library(
    name = "com_twitter_util_hashing_2_11",
    visibility = ["//visibility:public"],
    exports = ["@com_twitter_util_hashing_2_11//jar"],
    runtime_deps = [
      ":org_scala_lang_scala_library",
    ],
  )
  native.java_library(
    name = "com_twitter_util_jvm_2_11",
    visibility = ["//visibility:public"],
    exports = ["@com_twitter_util_jvm_2_11//jar"],
    runtime_deps = [
      ":org_scala_lang_scala_library",
      ":com_twitter_util_app_2_11",
      ":com_twitter_util_core_2_11",
    ],
  )
  native.java_library(
    name = "com_twitter_util_lint_2_11",
    visibility = ["//visibility:public"],
    exports = ["@com_twitter_util_lint_2_11//jar"],
    runtime_deps = [
      ":org_scala_lang_scala_library",
    ],
  )
  native.java_library(
    name = "com_twitter_util_logging_2_11",
    visibility = ["//visibility:public"],
    exports = ["@com_twitter_util_logging_2_11//jar"],
    runtime_deps = [
      ":org_scala_lang_scala_library",
      ":com_twitter_util_core_2_11",
      ":com_twitter_util_app_2_11",
      ":com_twitter_util_stats_2_11",
    ],
  )
  native.java_library(
    name = "com_twitter_util_registry_2_11",
    visibility = ["//visibility:public"],
    exports = ["@com_twitter_util_registry_2_11//jar"],
    runtime_deps = [
      ":org_scala_lang_scala_library",
      ":com_twitter_util_core_2_11",
    ],
  )
  native.java_library(
    name = "com_twitter_util_stats_2_11",
    visibility = ["//visibility:public"],
    exports = ["@com_twitter_util_stats_2_11//jar"],
    runtime_deps = [
      ":org_scala_lang_scala_library",
      ":com_twitter_util_core_2_11",
      ":com_twitter_util_lint_2_11",
    ],
  )
  native.java_library(
    name = "commons_beanutils_commons_beanutils",
    visibility = ["//visibility:public"],
    exports = ["@commons_beanutils_commons_beanutils//jar"],
    runtime_deps = [
      ":commons_logging_commons_logging",
      ":commons_collections_commons_collections",
    ],
  )
  native.java_library(
    name = "commons_cli_commons_cli",
    visibility = ["//visibility:public"],
    exports = ["@commons_cli_commons_cli//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "commons_codec_commons_codec",
    visibility = ["//visibility:public"],
    exports = ["@commons_codec_commons_codec//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "commons_collections_commons_collections",
    visibility = ["//visibility:public"],
    exports = ["@commons_collections_commons_collections//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "commons_configuration_commons_configuration",
    visibility = ["//visibility:public"],
    exports = ["@commons_configuration_commons_configuration//jar"],
    runtime_deps = [
      ":commons_lang_commons_lang",
    ],
  )
  native.java_library(
    name = "commons_io_commons_io",
    visibility = ["//visibility:public"],
    exports = ["@commons_io_commons_io//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "commons_lang_commons_lang",
    visibility = ["//visibility:public"],
    exports = ["@commons_lang_commons_lang//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "commons_logging_commons_logging",
    visibility = ["//visibility:public"],
    exports = ["@commons_logging_commons_logging//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "commons_net_commons_net",
    visibility = ["//visibility:public"],
    exports = ["@commons_net_commons_net//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "handle_net_handle_client",
    visibility = ["//visibility:public"],
    exports = ["@handle_net_handle_client//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "io_dropwizard_metrics_metrics_annotation",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_metrics_metrics_annotation//jar"],
    runtime_deps = [
      ":org_slf4j_slf4j_api",
    ],
  )
  native.java_library(
    name = "io_dropwizard_metrics_metrics_core",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_metrics_metrics_core//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "io_dropwizard_metrics_metrics_healthchecks",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_metrics_metrics_healthchecks//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "io_dropwizard_metrics_metrics_httpclient",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_metrics_metrics_httpclient//jar"],
    runtime_deps = [
      ":io_dropwizard_metrics_metrics_core",
    ],
  )
  native.java_library(
    name = "io_dropwizard_metrics_metrics_jersey2",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_metrics_metrics_jersey2//jar"],
    runtime_deps = [
      ":io_dropwizard_metrics_metrics_core",
      ":io_dropwizard_metrics_metrics_annotation",
    ],
  )
  native.java_library(
    name = "io_dropwizard_metrics_metrics_jetty9",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_metrics_metrics_jetty9//jar"],
    runtime_deps = [
      ":io_dropwizard_metrics_metrics_core",
    ],
  )
  native.java_library(
    name = "io_dropwizard_metrics_metrics_json",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_metrics_metrics_json//jar"],
    runtime_deps = [
      ":io_dropwizard_metrics_metrics_core",
    ],
  )
  native.java_library(
    name = "io_dropwizard_metrics_metrics_jvm",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_metrics_metrics_jvm//jar"],
    runtime_deps = [
      ":io_dropwizard_metrics_metrics_core",
    ],
  )
  native.java_library(
    name = "io_dropwizard_metrics_metrics_logback",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_metrics_metrics_logback//jar"],
    runtime_deps = [
      ":io_dropwizard_metrics_metrics_core",
    ],
  )
  native.java_library(
    name = "io_dropwizard_metrics_metrics_servlets",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_metrics_metrics_servlets//jar"],
    runtime_deps = [
      ":io_dropwizard_metrics_metrics_core",
      ":io_dropwizard_metrics_metrics_healthchecks",
      ":io_dropwizard_metrics_metrics_json",
      ":io_dropwizard_metrics_metrics_jvm",
      ":com_papertrail_profiler",
    ],
  )
  native.java_library(
    name = "io_dropwizard_dropwizard_assets",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_dropwizard_assets//jar"],
    runtime_deps = [
      ":io_dropwizard_dropwizard_core",
      ":io_dropwizard_dropwizard_servlets",
    ],
  )
  native.java_library(
    name = "io_dropwizard_dropwizard_client",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_dropwizard_client//jar"],
    runtime_deps = [
      ":io_dropwizard_dropwizard_core",
      ":org_glassfish_jersey_core_jersey_client",
      ":org_glassfish_jersey_ext_rx_jersey_rx_client",
      ":org_apache_httpcomponents_httpclient",
      ":io_dropwizard_metrics_metrics_httpclient",
      ":org_glassfish_jersey_connectors_jersey_apache_connector",
    ],
  )
  native.java_library(
    name = "io_dropwizard_dropwizard_configuration",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_dropwizard_configuration//jar"],
    runtime_deps = [
      ":io_dropwizard_dropwizard_jackson",
      ":io_dropwizard_dropwizard_validation",
      ":com_fasterxml_jackson_dataformat_jackson_dataformat_yaml",
      ":org_apache_commons_commons_lang3",
    ],
  )
  native.java_library(
    name = "io_dropwizard_dropwizard_core",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_dropwizard_core//jar"],
    runtime_deps = [
      ":io_dropwizard_dropwizard_util",
      ":io_dropwizard_dropwizard_jackson",
      ":io_dropwizard_dropwizard_validation",
      ":io_dropwizard_dropwizard_configuration",
      ":io_dropwizard_dropwizard_logging",
      ":io_dropwizard_dropwizard_metrics",
      ":io_dropwizard_dropwizard_jersey",
      ":io_dropwizard_dropwizard_servlets",
      ":io_dropwizard_dropwizard_jetty",
      ":io_dropwizard_dropwizard_lifecycle",
      ":io_dropwizard_metrics_metrics_core",
      ":io_dropwizard_metrics_metrics_jvm",
      ":io_dropwizard_metrics_metrics_servlets",
      ":io_dropwizard_metrics_metrics_healthchecks",
      ":io_dropwizard_dropwizard_request_logging",
      ":net_sourceforge_argparse4j_argparse4j",
      ":org_eclipse_jetty_toolchain_setuid_jetty_setuid_java",
    ],
  )
  native.java_library(
    name = "io_dropwizard_dropwizard_forms",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_dropwizard_forms//jar"],
    runtime_deps = [
      ":io_dropwizard_dropwizard_core",
      ":org_glassfish_jersey_media_jersey_media_multipart",
    ],
  )
  native.java_library(
    name = "io_dropwizard_dropwizard_jackson",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_dropwizard_jackson//jar"],
    runtime_deps = [
      ":com_google_guava_guava",
      ":io_dropwizard_dropwizard_util",
      ":com_fasterxml_jackson_core_jackson_core",
      ":com_fasterxml_jackson_core_jackson_annotations",
      ":com_fasterxml_jackson_core_jackson_databind",
      ":com_fasterxml_jackson_datatype_jackson_datatype_guava",
      ":com_fasterxml_jackson_datatype_jackson_datatype_jsr310",
      ":com_fasterxml_jackson_datatype_jackson_datatype_jdk8",
      ":com_fasterxml_jackson_module_jackson_module_parameter_names",
      ":com_fasterxml_jackson_module_jackson_module_afterburner",
      ":com_fasterxml_jackson_datatype_jackson_datatype_joda",
      ":org_slf4j_slf4j_api",
    ],
  )
  native.java_library(
    name = "io_dropwizard_dropwizard_jersey",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_dropwizard_jersey//jar"],
    runtime_deps = [
      ":io_dropwizard_dropwizard_jackson",
      ":io_dropwizard_dropwizard_validation",
      ":io_dropwizard_dropwizard_logging",
      ":org_glassfish_jersey_core_jersey_server",
      ":org_glassfish_jersey_ext_jersey_metainf_services",
      ":org_glassfish_jersey_ext_jersey_bean_validation",
      ":io_dropwizard_metrics_metrics_jersey2",
      ":com_fasterxml_jackson_jaxrs_jackson_jaxrs_json_provider",
      ":org_glassfish_jersey_containers_jersey_container_servlet",
      ":org_eclipse_jetty_jetty_server",
      ":org_eclipse_jetty_jetty_webapp",
      ":org_eclipse_jetty_jetty_continuation",
      ":org_apache_commons_commons_lang3",
    ],
  )
  native.java_library(
    name = "io_dropwizard_dropwizard_jetty",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_dropwizard_jetty//jar"],
    runtime_deps = [
      ":io_dropwizard_dropwizard_logging",
      ":io_dropwizard_metrics_metrics_jetty9",
      ":org_eclipse_jetty_jetty_server",
      ":org_eclipse_jetty_jetty_servlet",
      ":org_eclipse_jetty_jetty_servlets",
      ":org_eclipse_jetty_jetty_http",
    ],
  )
  native.java_library(
    name = "io_dropwizard_dropwizard_lifecycle",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_dropwizard_lifecycle//jar"],
    runtime_deps = [
      ":org_slf4j_slf4j_api",
      ":com_google_guava_guava",
      ":org_eclipse_jetty_jetty_server",
      ":io_dropwizard_dropwizard_util",
    ],
  )
  native.java_library(
    name = "io_dropwizard_dropwizard_logging",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_dropwizard_logging//jar"],
    runtime_deps = [
      ":io_dropwizard_dropwizard_jackson",
      ":io_dropwizard_dropwizard_validation",
      ":io_dropwizard_metrics_metrics_logback",
      ":org_slf4j_slf4j_api",
      ":org_slf4j_jul_to_slf4j",
      ":ch_qos_logback_logback_core",
      ":ch_qos_logback_logback_classic",
      ":org_slf4j_log4j_over_slf4j",
      ":org_slf4j_jcl_over_slf4j",
      ":org_eclipse_jetty_jetty_util",
    ],
  )
  native.java_library(
    name = "io_dropwizard_dropwizard_metrics",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_dropwizard_metrics//jar"],
    runtime_deps = [
      ":io_dropwizard_dropwizard_lifecycle",
      ":io_dropwizard_dropwizard_jackson",
      ":io_dropwizard_dropwizard_validation",
      ":io_dropwizard_metrics_metrics_core",
      ":org_slf4j_slf4j_api",
    ],
  )
  native.java_library(
    name = "io_dropwizard_dropwizard_request_logging",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_dropwizard_request_logging//jar"],
    runtime_deps = [
      ":io_dropwizard_dropwizard_jetty",
      ":io_dropwizard_dropwizard_logging",
      ":ch_qos_logback_logback_access",
    ],
  )
  native.java_library(
    name = "io_dropwizard_dropwizard_servlets",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_dropwizard_servlets//jar"],
    runtime_deps = [
      ":org_slf4j_slf4j_api",
      ":io_dropwizard_dropwizard_util",
      ":io_dropwizard_metrics_metrics_annotation",
      ":io_dropwizard_metrics_metrics_core",
      ":ch_qos_logback_logback_classic",
    ],
  )
  native.java_library(
    name = "io_dropwizard_dropwizard_testing",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_dropwizard_testing//jar"],
    runtime_deps = [
      ":io_dropwizard_dropwizard_core",
      ":junit_junit",
      ":org_objenesis_objenesis",
      ":org_assertj_assertj_core",
      ":org_glassfish_jersey_test_framework_providers_jersey_test_framework_provider_inmemory",
    ],
  )
  native.java_library(
    name = "io_dropwizard_dropwizard_util",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_dropwizard_util//jar"],
    runtime_deps = [
      ":com_fasterxml_jackson_core_jackson_annotations",
      ":com_google_guava_guava",
      ":com_google_code_findbugs_jsr305",
      ":joda_time_joda_time",
    ],
  )
  native.java_library(
    name = "io_dropwizard_dropwizard_validation",
    visibility = ["//visibility:public"],
    exports = ["@io_dropwizard_dropwizard_validation//jar"],
    runtime_deps = [
      ":io_dropwizard_dropwizard_util",
      ":org_hibernate_hibernate_validator",
      ":org_glassfish_javax_el",
    ],
  )
  native.java_library(
    name = "io_netty_netty",
    visibility = ["//visibility:public"],
    exports = ["@io_netty_netty//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "io_netty_netty_buffer",
    visibility = ["//visibility:public"],
    exports = ["@io_netty_netty_buffer//jar"],
    runtime_deps = [
      ":io_netty_netty_common",
    ],
  )
  native.java_library(
    name = "io_netty_netty_codec",
    visibility = ["//visibility:public"],
    exports = ["@io_netty_netty_codec//jar"],
    runtime_deps = [
      ":io_netty_netty_transport",
    ],
  )
  native.java_library(
    name = "io_netty_netty_codec_http",
    visibility = ["//visibility:public"],
    exports = ["@io_netty_netty_codec_http//jar"],
    runtime_deps = [
      ":io_netty_netty_codec",
      ":io_netty_netty_handler",
    ],
  )
  native.java_library(
    name = "io_netty_netty_codec_socks",
    visibility = ["//visibility:public"],
    exports = ["@io_netty_netty_codec_socks//jar"],
    runtime_deps = [
      ":io_netty_netty_codec",
      ":io_netty_netty_handler",
    ],
  )
  native.java_library(
    name = "io_netty_netty_common",
    visibility = ["//visibility:public"],
    exports = ["@io_netty_netty_common//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "io_netty_netty_handler",
    visibility = ["//visibility:public"],
    exports = ["@io_netty_netty_handler//jar"],
    runtime_deps = [
      ":io_netty_netty_buffer",
      ":io_netty_netty_transport",
      ":io_netty_netty_codec",
    ],
  )
  native.java_library(
    name = "io_netty_netty_transport",
    visibility = ["//visibility:public"],
    exports = ["@io_netty_netty_transport//jar"],
    runtime_deps = [
      ":io_netty_netty_buffer",
    ],
  )
  native.java_library(
    name = "janino_janino",
    visibility = ["//visibility:public"],
    exports = ["@janino_janino//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "javax_activation_activation",
    visibility = ["//visibility:public"],
    exports = ["@javax_activation_activation//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "javax_annotation_javax_annotation_api",
    visibility = ["//visibility:public"],
    exports = ["@javax_annotation_javax_annotation_api//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "javax_inject_javax_inject",
    visibility = ["//visibility:public"],
    exports = ["@javax_inject_javax_inject//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "javax_mail_mailapi",
    visibility = ["//visibility:public"],
    exports = ["@javax_mail_mailapi//jar"],
    runtime_deps = [
      ":javax_activation_activation",
    ],
  )
  native.java_library(
    name = "javax_servlet_javax_servlet_api",
    visibility = ["//visibility:public"],
    exports = ["@javax_servlet_javax_servlet_api//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "javax_validation_validation_api",
    visibility = ["//visibility:public"],
    exports = ["@javax_validation_validation_api//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "javax_ws_rs_javax_ws_rs_api",
    visibility = ["//visibility:public"],
    exports = ["@javax_ws_rs_javax_ws_rs_api//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "jline_jline",
    visibility = ["//visibility:public"],
    exports = ["@jline_jline//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "joda_time_joda_time",
    visibility = ["//visibility:public"],
    exports = ["@joda_time_joda_time//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "junit_junit",
    visibility = ["//visibility:public"],
    exports = ["@junit_junit//jar"],
    runtime_deps = [
      ":org_hamcrest_hamcrest_core",
    ],
  )
  native.java_library(
    name = "me_moocar_logback_gelf",
    visibility = ["//visibility:public"],
    exports = ["@me_moocar_logback_gelf//jar"],
    runtime_deps = [
      ":org_slf4j_slf4j_api",
      ":com_google_code_gson_gson",
    ],
  )
  native.java_library(
    name = "net_bytebuddy_byte_buddy",
    visibility = ["//visibility:public"],
    exports = ["@net_bytebuddy_byte_buddy//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "net_bytebuddy_byte_buddy_agent",
    visibility = ["//visibility:public"],
    exports = ["@net_bytebuddy_byte_buddy_agent//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "net_gini_dropwizard_dropwizard_gelf",
    visibility = ["//visibility:public"],
    exports = ["@net_gini_dropwizard_dropwizard_gelf//jar"],
    runtime_deps = [
      ":io_dropwizard_dropwizard_core",
      ":me_moocar_logback_gelf",
    ],
  )
  native.java_library(
    name = "net_sf_jopt_simple_jopt_simple",
    visibility = ["//visibility:public"],
    exports = ["@net_sf_jopt_simple_jopt_simple//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "net_sourceforge_argparse4j_argparse4j",
    visibility = ["//visibility:public"],
    exports = ["@net_sourceforge_argparse4j_argparse4j//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "nl_knaw_huygens_huygens_persistence",
    visibility = ["//visibility:public"],
    exports = ["@nl_knaw_huygens_huygens_persistence//jar"],
    runtime_deps = [
      ":commons_io_commons_io",
      ":com_google_guava_guava",
      ":handle_net_handle_client",
      ":org_slf4j_jcl_over_slf4j",
    ],
  )
  native.java_library(
    name = "nl_knaw_huygens_security_core",
    visibility = ["//visibility:public"],
    exports = ["@nl_knaw_huygens_security_core//jar"],
    runtime_deps = [
      ":com_google_guava_guava",
    ],
  )
  native.java_library(
    name = "ognl_ognl",
    visibility = ["//visibility:public"],
    exports = ["@ognl_ognl//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_antlr_antlr4_runtime",
    visibility = ["//visibility:public"],
    exports = ["@org_antlr_antlr4_runtime//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_apache_activemq_protobuf_activemq_protobuf",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_activemq_protobuf_activemq_protobuf//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_apache_activemq_activemq_broker",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_activemq_activemq_broker//jar"],
    runtime_deps = [
      ":org_apache_activemq_activemq_client",
      ":org_apache_activemq_activemq_openwire_legacy",
    ],
  )
  native.java_library(
    name = "org_apache_activemq_activemq_client",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_activemq_activemq_client//jar"],
    runtime_deps = [
      ":org_slf4j_slf4j_api",
      ":org_apache_geronimo_specs_geronimo_jms_1_1_spec",
      ":org_fusesource_hawtbuf_hawtbuf",
      ":org_apache_geronimo_specs_geronimo_j2ee_management_1_1_spec",
    ],
  )
  native.java_library(
    name = "org_apache_activemq_activemq_jms_pool",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_activemq_activemq_jms_pool//jar"],
    runtime_deps = [
      ":org_slf4j_slf4j_api",
      ":org_apache_geronimo_specs_geronimo_jms_1_1_spec",
      ":org_apache_geronimo_specs_geronimo_jta_1_0_1B_spec",
      ":org_apache_commons_commons_pool2",
    ],
  )
  native.java_library(
    name = "org_apache_activemq_activemq_kahadb_store",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_activemq_activemq_kahadb_store//jar"],
    runtime_deps = [
      ":org_apache_activemq_activemq_broker",
      ":org_apache_activemq_protobuf_activemq_protobuf",
      ":org_apache_geronimo_specs_geronimo_j2ee_management_1_1_spec",
      ":commons_net_commons_net",
    ],
  )
  native.java_library(
    name = "org_apache_activemq_activemq_openwire_legacy",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_activemq_activemq_openwire_legacy//jar"],
    runtime_deps = [
      ":org_apache_activemq_activemq_client",
    ],
  )
  native.java_library(
    name = "org_apache_commons_commons_compress",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_commons_commons_compress//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_apache_commons_commons_csv",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_commons_commons_csv//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_apache_commons_commons_jexl3",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_commons_commons_jexl3//jar"],
    runtime_deps = [
      ":commons_logging_commons_logging",
    ],
  )
  native.java_library(
    name = "org_apache_commons_commons_lang3",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_commons_commons_lang3//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_apache_commons_commons_pool2",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_commons_commons_pool2//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_apache_geronimo_specs_geronimo_j2ee_management_1_1_spec",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_geronimo_specs_geronimo_j2ee_management_1_1_spec//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_apache_geronimo_specs_geronimo_jms_1_1_spec",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_geronimo_specs_geronimo_jms_1_1_spec//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_apache_geronimo_specs_geronimo_jta_1_0_1B_spec",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_geronimo_specs_geronimo_jta_1_0_1B_spec//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_apache_httpcomponents_fluent_hc",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_httpcomponents_fluent_hc//jar"],
    runtime_deps = [
      ":org_apache_httpcomponents_httpclient",
    ],
  )
  native.java_library(
    name = "org_apache_httpcomponents_httpasyncclient",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_httpcomponents_httpasyncclient//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_apache_httpcomponents_httpclient",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_httpcomponents_httpclient//jar"],
    runtime_deps = [
      ":org_apache_httpcomponents_httpcore",
      ":commons_codec_commons_codec",
    ],
  )
  native.java_library(
    name = "org_apache_httpcomponents_httpclient_cache",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_httpcomponents_httpclient_cache//jar"],
    runtime_deps = [
      ":org_apache_httpcomponents_httpclient",
    ],
  )
  native.java_library(
    name = "org_apache_httpcomponents_httpclient_osgi",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_httpcomponents_httpclient_osgi//jar"],
    runtime_deps = [
      ":org_apache_httpcomponents_httpclient",
      ":commons_codec_commons_codec",
      ":org_apache_httpcomponents_httpmime",
      ":org_apache_httpcomponents_httpclient_cache",
      ":org_apache_httpcomponents_fluent_hc",
    ],
  )
  native.java_library(
    name = "org_apache_httpcomponents_httpcore",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_httpcomponents_httpcore//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_apache_httpcomponents_httpcore_nio",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_httpcomponents_httpcore_nio//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_apache_httpcomponents_httpcore_osgi",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_httpcomponents_httpcore_osgi//jar"],
    runtime_deps = [
      ":org_apache_httpcomponents_httpcore",
      ":org_apache_httpcomponents_httpcore_nio",
    ],
  )
  native.java_library(
    name = "org_apache_httpcomponents_httpmime",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_httpcomponents_httpmime//jar"],
    runtime_deps = [
      ":org_apache_httpcomponents_httpclient",
    ],
  )
  native.java_library(
    name = "org_apache_ivy_ivy",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_ivy_ivy//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_apache_jena_jena_arq",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_jena_jena_arq//jar"],
    runtime_deps = [
      ":org_apache_jena_jena_core",
      ":org_apache_jena_jena_shaded_guava",
      ":org_apache_httpcomponents_httpclient",
      ":com_github_jsonld_java_jsonld_java",
      ":org_apache_httpcomponents_httpclient_cache",
      ":org_apache_thrift_libthrift",
      ":org_slf4j_jcl_over_slf4j",
      ":org_apache_commons_commons_csv",
      ":org_apache_commons_commons_lang3",
      ":org_slf4j_slf4j_api",
    ],
  )
  native.java_library(
    name = "org_apache_jena_jena_base",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_jena_jena_base//jar"],
    runtime_deps = [
      ":org_apache_jena_jena_shaded_guava",
      ":org_apache_commons_commons_csv",
      ":org_apache_commons_commons_lang3",
      ":com_github_andrewoma_dexx_collection",
      ":org_slf4j_slf4j_api",
    ],
  )
  native.java_library(
    name = "org_apache_jena_jena_core",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_jena_jena_core//jar"],
    runtime_deps = [
      ":org_slf4j_slf4j_api",
      ":org_apache_jena_jena_iri",
      ":xerces_xercesImpl",
      ":commons_cli_commons_cli",
      ":org_apache_jena_jena_base",
    ],
  )
  native.java_library(
    name = "org_apache_jena_jena_iri",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_jena_jena_iri//jar"],
    runtime_deps = [
      ":org_slf4j_slf4j_api",
    ],
  )
  native.java_library(
    name = "org_apache_jena_jena_shaded_guava",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_jena_jena_shaded_guava//jar"],
    runtime_deps = [
      ":org_slf4j_slf4j_api",
    ],
  )
  native.java_library(
    name = "org_apache_jena_jena_tdb",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_jena_jena_tdb//jar"],
    runtime_deps = [
      ":org_apache_jena_jena_arq",
      ":org_slf4j_slf4j_api",
    ],
  )
  native.java_library(
    name = "org_apache_lucene_lucene_analyzers_common",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_lucene_lucene_analyzers_common//jar"],
    runtime_deps = [
      ":org_apache_lucene_lucene_core",
    ],
  )
  native.java_library(
    name = "org_apache_lucene_lucene_backward_codecs",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_lucene_lucene_backward_codecs//jar"],
    runtime_deps = [
      ":org_apache_lucene_lucene_core",
    ],
  )
  native.java_library(
    name = "org_apache_lucene_lucene_codecs",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_lucene_lucene_codecs//jar"],
    runtime_deps = [
      ":org_apache_lucene_lucene_core",
    ],
  )
  native.java_library(
    name = "org_apache_lucene_lucene_core",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_lucene_lucene_core//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_apache_lucene_lucene_queryparser",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_lucene_lucene_queryparser//jar"],
    runtime_deps = [
      ":org_apache_lucene_lucene_core",
    ],
  )
  native.java_library(
    name = "org_apache_poi_poi",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_poi_poi//jar"],
    runtime_deps = [
      ":commons_codec_commons_codec",
    ],
  )
  native.java_library(
    name = "org_apache_poi_poi_ooxml",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_poi_poi_ooxml//jar"],
    runtime_deps = [
      ":org_apache_poi_poi",
      ":org_apache_poi_poi_ooxml_schemas",
      ":com_github_virtuald_curvesapi",
    ],
  )
  native.java_library(
    name = "org_apache_poi_poi_ooxml_schemas",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_poi_poi_ooxml_schemas//jar"],
    runtime_deps = [
      ":org_apache_xmlbeans_xmlbeans",
    ],
  )
  native.java_library(
    name = "org_apache_thrift_libthrift",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_thrift_libthrift//jar"],
    runtime_deps = [
      ":org_slf4j_slf4j_api",
      ":org_apache_httpcomponents_httpclient",
    ],
  )
  native.java_library(
    name = "org_apache_tinkerpop_gremlin_core",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_tinkerpop_gremlin_core//jar"],
    runtime_deps = [
      ":org_apache_tinkerpop_gremlin_shaded",
      ":commons_configuration_commons_configuration",
      ":org_yaml_snakeyaml",
      ":org_javatuples_javatuples",
      ":com_carrotsearch_hppc",
      ":com_jcabi_jcabi_manifests",
      ":org_slf4j_jcl_over_slf4j",
    ],
  )
  native.java_library(
    name = "org_apache_tinkerpop_gremlin_groovy",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_tinkerpop_gremlin_groovy//jar"],
    runtime_deps = [
      ":org_apache_tinkerpop_gremlin_core",
      ":org_apache_ivy_ivy",
      ":org_codehaus_groovy_groovy",
      ":org_codehaus_groovy_groovy_groovysh",
      ":org_codehaus_groovy_groovy_json",
      ":org_codehaus_groovy_groovy_jsr223",
      ":org_apache_commons_commons_lang3",
      ":org_mindrot_jbcrypt",
    ],
  )
  native.java_library(
    name = "org_apache_tinkerpop_gremlin_shaded",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_tinkerpop_gremlin_shaded//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_apache_tinkerpop_neo4j_gremlin",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_tinkerpop_neo4j_gremlin//jar"],
    runtime_deps = [
      ":org_apache_tinkerpop_gremlin_core",
      ":org_apache_tinkerpop_gremlin_groovy",
      ":org_neo4j_neo4j_tinkerpop_api",
    ],
  )
  native.java_library(
    name = "org_apache_xmlbeans_xmlbeans",
    visibility = ["//visibility:public"],
    exports = ["@org_apache_xmlbeans_xmlbeans//jar"],
    runtime_deps = [
      ":stax_stax_api",
    ],
  )
  native.java_library(
    name = "org_assertj_assertj_core",
    visibility = ["//visibility:public"],
    exports = ["@org_assertj_assertj_core//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_bouncycastle_bcmail_jdk15on",
    visibility = ["//visibility:public"],
    exports = ["@org_bouncycastle_bcmail_jdk15on//jar"],
    runtime_deps = [
      ":org_bouncycastle_bcprov_jdk15on",
      ":org_bouncycastle_bcpkix_jdk15on",
    ],
  )
  native.java_library(
    name = "org_bouncycastle_bcpkix_jdk15on",
    visibility = ["//visibility:public"],
    exports = ["@org_bouncycastle_bcpkix_jdk15on//jar"],
    runtime_deps = [
      ":org_bouncycastle_bcprov_jdk15on",
    ],
  )
  native.java_library(
    name = "org_bouncycastle_bcprov_jdk15on",
    visibility = ["//visibility:public"],
    exports = ["@org_bouncycastle_bcprov_jdk15on//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_codehaus_groovy_groovy",
    visibility = ["//visibility:public"],
    exports = ["@org_codehaus_groovy_groovy//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_codehaus_groovy_groovy_console",
    visibility = ["//visibility:public"],
    exports = ["@org_codehaus_groovy_groovy_console//jar"],
    runtime_deps = [
      ":org_codehaus_groovy_groovy_templates",
      ":org_codehaus_groovy_groovy_swing",
      ":org_codehaus_groovy_groovy",
    ],
  )
  native.java_library(
    name = "org_codehaus_groovy_groovy_groovysh",
    visibility = ["//visibility:public"],
    exports = ["@org_codehaus_groovy_groovy_groovysh//jar"],
    runtime_deps = [
      ":org_codehaus_groovy_groovy_console",
      ":jline_jline",
      ":org_codehaus_groovy_groovy",
    ],
  )
  native.java_library(
    name = "org_codehaus_groovy_groovy_json",
    visibility = ["//visibility:public"],
    exports = ["@org_codehaus_groovy_groovy_json//jar"],
    runtime_deps = [
      ":org_codehaus_groovy_groovy",
    ],
  )
  native.java_library(
    name = "org_codehaus_groovy_groovy_jsr223",
    visibility = ["//visibility:public"],
    exports = ["@org_codehaus_groovy_groovy_jsr223//jar"],
    runtime_deps = [
      ":org_codehaus_groovy_groovy",
    ],
  )
  native.java_library(
    name = "org_codehaus_groovy_groovy_swing",
    visibility = ["//visibility:public"],
    exports = ["@org_codehaus_groovy_groovy_swing//jar"],
    runtime_deps = [
      ":org_codehaus_groovy_groovy",
    ],
  )
  native.java_library(
    name = "org_codehaus_groovy_groovy_templates",
    visibility = ["//visibility:public"],
    exports = ["@org_codehaus_groovy_groovy_templates//jar"],
    runtime_deps = [
      ":org_codehaus_groovy_groovy",
      ":org_codehaus_groovy_groovy_xml",
    ],
  )
  native.java_library(
    name = "org_codehaus_groovy_groovy_xml",
    visibility = ["//visibility:public"],
    exports = ["@org_codehaus_groovy_groovy_xml//jar"],
    runtime_deps = [
      ":org_codehaus_groovy_groovy",
    ],
  )
  native.java_library(
    name = "org_codehaus_woodstox_stax2_api",
    visibility = ["//visibility:public"],
    exports = ["@org_codehaus_woodstox_stax2_api//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_concordion_concordion",
    visibility = ["//visibility:public"],
    exports = ["@org_concordion_concordion//jar"],
    runtime_deps = [
      ":junit_junit",
      ":xom_xom",
      ":ognl_ognl",
    ],
  )
  native.java_library(
    name = "org_eclipse_jetty_toolchain_setuid_jetty_setuid_java",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_jetty_toolchain_setuid_jetty_setuid_java//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_eclipse_jetty_jetty_continuation",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_jetty_jetty_continuation//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_eclipse_jetty_jetty_http",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_jetty_jetty_http//jar"],
    runtime_deps = [
      ":org_eclipse_jetty_jetty_util",
      ":org_eclipse_jetty_jetty_io",
    ],
  )
  native.java_library(
    name = "org_eclipse_jetty_jetty_io",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_jetty_jetty_io//jar"],
    runtime_deps = [
      ":org_eclipse_jetty_jetty_util",
    ],
  )
  native.java_library(
    name = "org_eclipse_jetty_jetty_security",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_jetty_jetty_security//jar"],
    runtime_deps = [
      ":org_eclipse_jetty_jetty_server",
    ],
  )
  native.java_library(
    name = "org_eclipse_jetty_jetty_server",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_jetty_jetty_server//jar"],
    runtime_deps = [
      ":javax_servlet_javax_servlet_api",
      ":org_eclipse_jetty_jetty_http",
      ":org_eclipse_jetty_jetty_io",
    ],
  )
  native.java_library(
    name = "org_eclipse_jetty_jetty_servlet",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_jetty_jetty_servlet//jar"],
    runtime_deps = [
      ":org_eclipse_jetty_jetty_security",
    ],
  )
  native.java_library(
    name = "org_eclipse_jetty_jetty_servlets",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_jetty_jetty_servlets//jar"],
    runtime_deps = [
      ":org_eclipse_jetty_jetty_continuation",
      ":org_eclipse_jetty_jetty_http",
      ":org_eclipse_jetty_jetty_util",
      ":org_eclipse_jetty_jetty_io",
    ],
  )
  native.java_library(
    name = "org_eclipse_jetty_jetty_util",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_jetty_jetty_util//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_eclipse_jetty_jetty_webapp",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_jetty_jetty_webapp//jar"],
    runtime_deps = [
      ":org_eclipse_jetty_jetty_xml",
      ":org_eclipse_jetty_jetty_servlet",
    ],
  )
  native.java_library(
    name = "org_eclipse_jetty_jetty_xml",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_jetty_jetty_xml//jar"],
    runtime_deps = [
      ":org_eclipse_jetty_jetty_util",
    ],
  )
  native.java_library(
    name = "org_eclipse_rdf4j_rdf4j_model",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_rdf4j_rdf4j_model//jar"],
    runtime_deps = [
      ":org_eclipse_rdf4j_rdf4j_util",
    ],
  )
  native.java_library(
    name = "org_eclipse_rdf4j_rdf4j_rio_api",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_rdf4j_rdf4j_rio_api//jar"],
    runtime_deps = [
      ":org_eclipse_rdf4j_rdf4j_model",
      ":org_eclipse_rdf4j_rdf4j_util",
    ],
  )
  native.java_library(
    name = "org_eclipse_rdf4j_rdf4j_rio_datatypes",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_rdf4j_rdf4j_rio_datatypes//jar"],
    runtime_deps = [
      ":org_eclipse_rdf4j_rdf4j_model",
      ":org_eclipse_rdf4j_rdf4j_rio_api",
      ":org_eclipse_rdf4j_rdf4j_util",
    ],
  )
  native.java_library(
    name = "org_eclipse_rdf4j_rdf4j_rio_jsonld",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_rdf4j_rdf4j_rio_jsonld//jar"],
    runtime_deps = [
      ":org_eclipse_rdf4j_rdf4j_model",
      ":org_eclipse_rdf4j_rdf4j_rio_api",
      ":org_eclipse_rdf4j_rdf4j_rio_datatypes",
      ":org_eclipse_rdf4j_rdf4j_rio_languages",
      ":org_eclipse_rdf4j_rdf4j_util",
      ":com_github_jsonld_java_jsonld_java",
      ":com_fasterxml_jackson_core_jackson_core",
      ":commons_io_commons_io",
    ],
  )
  native.java_library(
    name = "org_eclipse_rdf4j_rdf4j_rio_languages",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_rdf4j_rdf4j_rio_languages//jar"],
    runtime_deps = [
      ":org_eclipse_rdf4j_rdf4j_model",
      ":org_eclipse_rdf4j_rdf4j_rio_api",
      ":org_eclipse_rdf4j_rdf4j_util",
    ],
  )
  native.java_library(
    name = "org_eclipse_rdf4j_rdf4j_rio_n3",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_rdf4j_rdf4j_rio_n3//jar"],
    runtime_deps = [
      ":org_eclipse_rdf4j_rdf4j_model",
      ":org_eclipse_rdf4j_rdf4j_rio_api",
      ":org_eclipse_rdf4j_rdf4j_rio_turtle",
      ":org_eclipse_rdf4j_rdf4j_rio_datatypes",
      ":org_eclipse_rdf4j_rdf4j_rio_languages",
    ],
  )
  native.java_library(
    name = "org_eclipse_rdf4j_rdf4j_rio_nquads",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_rdf4j_rdf4j_rio_nquads//jar"],
    runtime_deps = [
      ":org_eclipse_rdf4j_rdf4j_model",
      ":org_eclipse_rdf4j_rdf4j_rio_api",
      ":org_eclipse_rdf4j_rdf4j_rio_ntriples",
      ":org_eclipse_rdf4j_rdf4j_rio_datatypes",
      ":org_eclipse_rdf4j_rdf4j_rio_languages",
      ":commons_io_commons_io",
    ],
  )
  native.java_library(
    name = "org_eclipse_rdf4j_rdf4j_rio_ntriples",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_rdf4j_rdf4j_rio_ntriples//jar"],
    runtime_deps = [
      ":org_eclipse_rdf4j_rdf4j_model",
      ":org_eclipse_rdf4j_rdf4j_rio_api",
      ":org_eclipse_rdf4j_rdf4j_rio_datatypes",
      ":org_eclipse_rdf4j_rdf4j_rio_languages",
      ":commons_io_commons_io",
    ],
  )
  native.java_library(
    name = "org_eclipse_rdf4j_rdf4j_rio_rdfxml",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_rdf4j_rdf4j_rio_rdfxml//jar"],
    runtime_deps = [
      ":org_eclipse_rdf4j_rdf4j_model",
      ":org_eclipse_rdf4j_rdf4j_rio_api",
      ":org_eclipse_rdf4j_rdf4j_rio_datatypes",
      ":org_eclipse_rdf4j_rdf4j_rio_languages",
      ":org_eclipse_rdf4j_rdf4j_util",
      ":commons_io_commons_io",
    ],
  )
  native.java_library(
    name = "org_eclipse_rdf4j_rdf4j_rio_trig",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_rdf4j_rdf4j_rio_trig//jar"],
    runtime_deps = [
      ":org_eclipse_rdf4j_rdf4j_model",
      ":org_eclipse_rdf4j_rdf4j_rio_api",
      ":org_eclipse_rdf4j_rdf4j_rio_turtle",
      ":org_eclipse_rdf4j_rdf4j_rio_datatypes",
      ":org_eclipse_rdf4j_rdf4j_rio_languages",
      ":org_eclipse_rdf4j_rdf4j_util",
    ],
  )
  native.java_library(
    name = "org_eclipse_rdf4j_rdf4j_rio_turtle",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_rdf4j_rdf4j_rio_turtle//jar"],
    runtime_deps = [
      ":org_eclipse_rdf4j_rdf4j_model",
      ":org_eclipse_rdf4j_rdf4j_rio_api",
      ":org_eclipse_rdf4j_rdf4j_rio_datatypes",
      ":org_eclipse_rdf4j_rdf4j_rio_languages",
      ":org_eclipse_rdf4j_rdf4j_util",
      ":commons_io_commons_io",
    ],
  )
  native.java_library(
    name = "org_eclipse_rdf4j_rdf4j_util",
    visibility = ["//visibility:public"],
    exports = ["@org_eclipse_rdf4j_rdf4j_util//jar"],
    runtime_deps = [
      ":com_google_guava_guava",
      ":org_slf4j_slf4j_api",
    ],
  )
  native.java_library(
    name = "org_elasticsearch_client_rest",
    visibility = ["//visibility:public"],
    exports = ["@org_elasticsearch_client_rest//jar"],
    runtime_deps = [
      ":org_apache_httpcomponents_httpclient",
      ":org_apache_httpcomponents_httpcore",
      ":org_apache_httpcomponents_httpasyncclient",
      ":org_apache_httpcomponents_httpcore_nio",
      ":commons_codec_commons_codec",
      ":commons_logging_commons_logging",
    ],
  )
  native.java_library(
    name = "org_fusesource_hawtbuf_hawtbuf",
    visibility = ["//visibility:public"],
    exports = ["@org_fusesource_hawtbuf_hawtbuf//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_glassfish_hk2_external_aopalliance_repackaged",
    visibility = ["//visibility:public"],
    exports = ["@org_glassfish_hk2_external_aopalliance_repackaged//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_glassfish_hk2_external_javax_inject",
    visibility = ["//visibility:public"],
    exports = ["@org_glassfish_hk2_external_javax_inject//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_glassfish_hk2_hk2_api",
    visibility = ["//visibility:public"],
    exports = ["@org_glassfish_hk2_hk2_api//jar"],
    runtime_deps = [
      ":org_glassfish_hk2_hk2_utils",
      ":org_glassfish_hk2_external_aopalliance_repackaged",
    ],
  )
  native.java_library(
    name = "org_glassfish_hk2_hk2_locator",
    visibility = ["//visibility:public"],
    exports = ["@org_glassfish_hk2_hk2_locator//jar"],
    runtime_deps = [
      ":org_glassfish_hk2_external_javax_inject",
      ":org_glassfish_hk2_external_aopalliance_repackaged",
      ":org_glassfish_hk2_hk2_api",
      ":org_glassfish_hk2_hk2_utils",
      ":org_javassist_javassist",
    ],
  )
  native.java_library(
    name = "org_glassfish_hk2_hk2_utils",
    visibility = ["//visibility:public"],
    exports = ["@org_glassfish_hk2_hk2_utils//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_glassfish_hk2_osgi_resource_locator",
    visibility = ["//visibility:public"],
    exports = ["@org_glassfish_hk2_osgi_resource_locator//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_glassfish_jaxb_txw2",
    visibility = ["//visibility:public"],
    exports = ["@org_glassfish_jaxb_txw2//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_glassfish_jersey_bundles_repackaged_jersey_guava",
    visibility = ["//visibility:public"],
    exports = ["@org_glassfish_jersey_bundles_repackaged_jersey_guava//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_glassfish_jersey_connectors_jersey_apache_connector",
    visibility = ["//visibility:public"],
    exports = ["@org_glassfish_jersey_connectors_jersey_apache_connector//jar"],
    runtime_deps = [
      ":org_apache_httpcomponents_httpclient",
      ":org_glassfish_jersey_core_jersey_common",
      ":org_glassfish_jersey_core_jersey_client",
      ":javax_ws_rs_javax_ws_rs_api",
    ],
  )
  native.java_library(
    name = "org_glassfish_jersey_containers_jersey_container_servlet",
    visibility = ["//visibility:public"],
    exports = ["@org_glassfish_jersey_containers_jersey_container_servlet//jar"],
    runtime_deps = [
      ":org_glassfish_jersey_containers_jersey_container_servlet_core",
      ":org_glassfish_jersey_core_jersey_common",
      ":org_glassfish_jersey_core_jersey_server",
      ":javax_ws_rs_javax_ws_rs_api",
    ],
  )
  native.java_library(
    name = "org_glassfish_jersey_containers_jersey_container_servlet_core",
    visibility = ["//visibility:public"],
    exports = ["@org_glassfish_jersey_containers_jersey_container_servlet_core//jar"],
    runtime_deps = [
      ":org_glassfish_hk2_external_javax_inject",
      ":org_glassfish_jersey_core_jersey_common",
      ":org_glassfish_jersey_core_jersey_server",
      ":javax_ws_rs_javax_ws_rs_api",
    ],
  )
  native.java_library(
    name = "org_glassfish_jersey_core_jersey_client",
    visibility = ["//visibility:public"],
    exports = ["@org_glassfish_jersey_core_jersey_client//jar"],
    runtime_deps = [
      ":javax_ws_rs_javax_ws_rs_api",
      ":org_glassfish_jersey_core_jersey_common",
      ":org_glassfish_hk2_hk2_api",
      ":org_glassfish_hk2_external_javax_inject",
      ":org_glassfish_hk2_hk2_locator",
    ],
  )
  native.java_library(
    name = "org_glassfish_jersey_core_jersey_common",
    visibility = ["//visibility:public"],
    exports = ["@org_glassfish_jersey_core_jersey_common//jar"],
    runtime_deps = [
      ":javax_ws_rs_javax_ws_rs_api",
      ":javax_annotation_javax_annotation_api",
      ":org_glassfish_jersey_bundles_repackaged_jersey_guava",
      ":org_glassfish_hk2_hk2_api",
      ":org_glassfish_hk2_external_javax_inject",
      ":org_glassfish_hk2_hk2_locator",
      ":org_glassfish_hk2_osgi_resource_locator",
    ],
  )
  native.java_library(
    name = "org_glassfish_jersey_core_jersey_server",
    visibility = ["//visibility:public"],
    exports = ["@org_glassfish_jersey_core_jersey_server//jar"],
    runtime_deps = [
      ":org_glassfish_jersey_core_jersey_common",
      ":org_glassfish_jersey_core_jersey_client",
      ":javax_ws_rs_javax_ws_rs_api",
      ":org_glassfish_jersey_media_jersey_media_jaxb",
      ":javax_annotation_javax_annotation_api",
      ":org_glassfish_hk2_hk2_api",
      ":org_glassfish_hk2_external_javax_inject",
      ":org_glassfish_hk2_hk2_locator",
      ":javax_validation_validation_api",
    ],
  )
  native.java_library(
    name = "org_glassfish_jersey_ext_rx_jersey_rx_client",
    visibility = ["//visibility:public"],
    exports = ["@org_glassfish_jersey_ext_rx_jersey_rx_client//jar"],
    runtime_deps = [
      ":org_glassfish_jersey_core_jersey_client",
      ":javax_ws_rs_javax_ws_rs_api",
    ],
  )
  native.java_library(
    name = "org_glassfish_jersey_ext_jersey_bean_validation",
    visibility = ["//visibility:public"],
    exports = ["@org_glassfish_jersey_ext_jersey_bean_validation//jar"],
    runtime_deps = [
      ":org_glassfish_hk2_external_javax_inject",
      ":org_glassfish_jersey_core_jersey_common",
      ":org_glassfish_jersey_core_jersey_server",
      ":javax_validation_validation_api",
      ":org_hibernate_hibernate_validator",
      ":javax_ws_rs_javax_ws_rs_api",
    ],
  )
  native.java_library(
    name = "org_glassfish_jersey_ext_jersey_metainf_services",
    visibility = ["//visibility:public"],
    exports = ["@org_glassfish_jersey_ext_jersey_metainf_services//jar"],
    runtime_deps = [
      ":org_glassfish_jersey_core_jersey_common",
      ":javax_ws_rs_javax_ws_rs_api",
    ],
  )
  native.java_library(
    name = "org_glassfish_jersey_media_jersey_media_jaxb",
    visibility = ["//visibility:public"],
    exports = ["@org_glassfish_jersey_media_jersey_media_jaxb//jar"],
    runtime_deps = [
      ":org_glassfish_jersey_core_jersey_common",
      ":org_glassfish_hk2_hk2_api",
      ":org_glassfish_hk2_external_javax_inject",
      ":org_glassfish_hk2_hk2_locator",
      ":org_glassfish_hk2_osgi_resource_locator",
    ],
  )
  native.java_library(
    name = "org_glassfish_jersey_media_jersey_media_multipart",
    visibility = ["//visibility:public"],
    exports = ["@org_glassfish_jersey_media_jersey_media_multipart//jar"],
    runtime_deps = [
      ":org_glassfish_jersey_core_jersey_common",
      ":org_jvnet_mimepull_mimepull",
    ],
  )
  native.java_library(
    name = "org_glassfish_jersey_test_framework_providers_jersey_test_framework_provider_inmemory",
    visibility = ["//visibility:public"],
    exports = ["@org_glassfish_jersey_test_framework_providers_jersey_test_framework_provider_inmemory//jar"],
    runtime_deps = [
      ":org_glassfish_jersey_test_framework_jersey_test_framework_core",
      ":org_glassfish_jersey_core_jersey_server",
      ":org_glassfish_jersey_core_jersey_client",
      ":junit_junit",
    ],
  )
  native.java_library(
    name = "org_glassfish_jersey_test_framework_jersey_test_framework_core",
    visibility = ["//visibility:public"],
    exports = ["@org_glassfish_jersey_test_framework_jersey_test_framework_core//jar"],
    runtime_deps = [
      ":org_glassfish_jersey_core_jersey_server",
      ":org_glassfish_jersey_containers_jersey_container_servlet_core",
      ":junit_junit",
    ],
  )
  native.java_library(
    name = "org_glassfish_javax_el",
    visibility = ["//visibility:public"],
    exports = ["@org_glassfish_javax_el//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_hamcrest_hamcrest_all",
    visibility = ["//visibility:public"],
    exports = ["@org_hamcrest_hamcrest_all//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_hamcrest_hamcrest_core",
    visibility = ["//visibility:public"],
    exports = ["@org_hamcrest_hamcrest_core//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_hibernate_hibernate_validator",
    visibility = ["//visibility:public"],
    exports = ["@org_hibernate_hibernate_validator//jar"],
    runtime_deps = [
      ":javax_validation_validation_api",
      ":org_jboss_logging_jboss_logging",
      ":com_fasterxml_classmate",
    ],
  )
  native.java_library(
    name = "org_immutables_value",
    visibility = ["//visibility:public"],
    exports = ["@org_immutables_value//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_javassist_javassist",
    visibility = ["//visibility:public"],
    exports = ["@org_javassist_javassist//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_javatuples_javatuples",
    visibility = ["//visibility:public"],
    exports = ["@org_javatuples_javatuples//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_jboss_logging_jboss_logging",
    visibility = ["//visibility:public"],
    exports = ["@org_jboss_logging_jboss_logging//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_json_json",
    visibility = ["//visibility:public"],
    exports = ["@org_json_json//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_jsoup_jsoup",
    visibility = ["//visibility:public"],
    exports = ["@org_jsoup_jsoup//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_jvnet_mimepull_mimepull",
    visibility = ["//visibility:public"],
    exports = ["@org_jvnet_mimepull_mimepull//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_mindrot_jbcrypt",
    visibility = ["//visibility:public"],
    exports = ["@org_mindrot_jbcrypt//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_mock_server_mockserver_client_java",
    visibility = ["//visibility:public"],
    exports = ["@org_mock_server_mockserver_client_java//jar"],
    runtime_deps = [
      ":org_mock_server_mockserver_core",
      ":org_apache_commons_commons_lang3",
      ":com_google_guava_guava",
      ":org_slf4j_slf4j_api",
      ":ch_qos_logback_logback_classic",
    ],
  )
  native.java_library(
    name = "org_mock_server_mockserver_core",
    visibility = ["//visibility:public"],
    exports = ["@org_mock_server_mockserver_core//jar"],
    runtime_deps = [
      ":javax_servlet_javax_servlet_api",
      ":io_netty_netty_buffer",
      ":io_netty_netty_codec",
      ":io_netty_netty_codec_http",
      ":io_netty_netty_codec_socks",
      ":io_netty_netty_handler",
      ":io_netty_netty_transport",
      ":com_twitter_finagle_native_2_11",
      ":com_jcraft_jzlib",
      ":org_bouncycastle_bcprov_jdk15on",
      ":org_bouncycastle_bcmail_jdk15on",
      ":org_bouncycastle_bcpkix_jdk15on",
      ":com_fasterxml_jackson_core_jackson_core",
      ":com_fasterxml_jackson_core_jackson_annotations",
      ":com_fasterxml_jackson_core_jackson_databind",
      ":org_skyscreamer_jsonassert",
      ":com_github_fge_json_schema_validator",
      ":org_apache_commons_commons_lang3",
      ":commons_io_commons_io",
      ":com_google_guava_guava",
      ":org_slf4j_slf4j_api",
      ":ch_qos_logback_logback_classic",
    ],
  )
  native.java_library(
    name = "org_mock_server_mockserver_logging",
    visibility = ["//visibility:public"],
    exports = ["@org_mock_server_mockserver_logging//jar"],
    runtime_deps = [
      ":org_slf4j_slf4j_api",
      ":ch_qos_logback_logback_classic",
    ],
  )
  native.java_library(
    name = "org_mock_server_mockserver_netty",
    visibility = ["//visibility:public"],
    exports = ["@org_mock_server_mockserver_netty//jar"],
    runtime_deps = [
      ":org_mock_server_mockserver_client_java",
      ":org_mock_server_mockserver_core",
      ":io_netty_netty_buffer",
      ":io_netty_netty_codec",
      ":io_netty_netty_codec_http",
      ":io_netty_netty_common",
      ":io_netty_netty_handler",
      ":io_netty_netty_transport",
      ":org_bouncycastle_bcprov_jdk15on",
      ":com_google_guava_guava",
      ":org_slf4j_slf4j_api",
      ":ch_qos_logback_logback_classic",
      ":janino_janino",
      ":org_mock_server_mockserver_logging",
      ":junit_junit",
    ],
  )
  native.java_library(
    name = "org_mockito_mockito_core",
    visibility = ["//visibility:public"],
    exports = ["@org_mockito_mockito_core//jar"],
    runtime_deps = [
      ":net_bytebuddy_byte_buddy",
      ":net_bytebuddy_byte_buddy_agent",
      ":org_objenesis_objenesis",
    ],
  )
  native.java_library(
    name = "org_mozilla_rhino",
    visibility = ["//visibility:public"],
    exports = ["@org_mozilla_rhino//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_backup",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_backup//jar"],
    runtime_deps = [
      ":org_neo4j_neo4j_com",
      ":org_neo4j_neo4j_command_line",
      ":org_neo4j_neo4j_dbms",
      ":org_neo4j_neo4j_cluster",
      ":org_neo4j_neo4j_consistency_check",
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_cluster",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_cluster//jar"],
    runtime_deps = [
      ":org_neo4j_neo4j_kernel",
      ":io_netty_netty",
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_collections",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_collections//jar"],
    runtime_deps = [
      ":org_neo4j_neo4j_resource",
      ":org_neo4j_neo4j_common",
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_com",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_com//jar"],
    runtime_deps = [
      ":org_neo4j_neo4j_kernel",
      ":org_neo4j_neo4j_enterprise_kernel",
      ":io_netty_netty",
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_command_line",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_command_line//jar"],
    runtime_deps = [
      ":org_neo4j_neo4j_kernel",
      ":org_neo4j_neo4j_collections",
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_common",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_common//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_consistency_check",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_consistency_check//jar"],
    runtime_deps = [
      ":org_neo4j_neo4j_kernel",
      ":org_neo4j_neo4j_lucene_index",
      ":org_neo4j_neo4j_command_line",
      ":org_neo4j_neo4j_dbms",
      ":org_neo4j_neo4j_io",
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_csv",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_csv//jar"],
    runtime_deps = [
      ":org_neo4j_neo4j_primitive_collections",
      ":org_neo4j_neo4j_collections",
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_dbms",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_dbms//jar"],
    runtime_deps = [
      ":org_neo4j_neo4j_kernel",
      ":org_neo4j_neo4j_graphdb_api",
      ":org_neo4j_neo4j_command_line",
      ":org_neo4j_neo4j_collections",
      ":org_neo4j_neo4j_io",
      ":org_neo4j_neo4j_import_tool",
      ":org_apache_commons_commons_compress",
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_enterprise_kernel",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_enterprise_kernel//jar"],
    runtime_deps = [
      ":org_neo4j_neo4j_kernel",
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_graphdb_api",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_graphdb_api//jar"],
    runtime_deps = [
      ":org_neo4j_neo4j_common",
      ":org_neo4j_neo4j_resource",
      ":org_neo4j_neo4j_collections",
      ":org_neo4j_neo4j_primitive_collections",
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_ha",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_ha//jar"],
    runtime_deps = [
      ":org_neo4j_neo4j_enterprise_kernel",
      ":org_neo4j_neo4j_backup",
      ":org_neo4j_neo4j_lucene_index",
      ":org_neo4j_neo4j_com",
      ":org_neo4j_neo4j_cluster",
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_import_tool",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_import_tool//jar"],
    runtime_deps = [
      ":org_neo4j_neo4j_kernel",
      ":org_neo4j_neo4j_lucene_index",
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_io",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_io//jar"],
    runtime_deps = [
      ":org_neo4j_neo4j_unsafe",
      ":org_neo4j_neo4j_primitive_collections",
      ":org_apache_commons_commons_lang3",
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_io_test_jar",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_io_test_jar//jar"],
    runtime_deps = [
      ":org_neo4j_neo4j_unsafe",
      ":org_neo4j_neo4j_primitive_collections",
      ":org_apache_commons_commons_lang3",
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_kernel",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_kernel//jar"],
    runtime_deps = [
      ":org_neo4j_neo4j_graphdb_api",
      ":org_neo4j_neo4j_common",
      ":org_neo4j_neo4j_collections",
      ":org_neo4j_neo4j_primitive_collections",
      ":org_neo4j_neo4j_io",
      ":org_neo4j_neo4j_csv",
      ":org_neo4j_neo4j_logging",
      ":org_neo4j_neo4j_lucene_upgrade",
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_kernel_test_jar",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_kernel_test_jar//jar"],
    runtime_deps = [
      ":org_neo4j_neo4j_graphdb_api",
      ":org_neo4j_neo4j_common",
      ":org_neo4j_neo4j_collections",
      ":org_neo4j_neo4j_primitive_collections",
      ":org_neo4j_neo4j_io",
      ":org_neo4j_neo4j_csv",
      ":org_neo4j_neo4j_logging",
      ":org_neo4j_neo4j_lucene_upgrade",
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_logging",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_logging//jar"],
    runtime_deps = [
      ":org_neo4j_neo4j_io",
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_lucene_index",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_lucene_index//jar"],
    runtime_deps = [
      ":org_neo4j_neo4j_kernel",
      ":org_apache_lucene_lucene_analyzers_common",
      ":org_apache_lucene_lucene_core",
      ":org_apache_lucene_lucene_queryparser",
      ":org_apache_lucene_lucene_codecs",
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_lucene_upgrade",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_lucene_upgrade//jar"],
    runtime_deps = [
      ":org_apache_lucene_lucene_core",
      ":org_apache_lucene_lucene_backward_codecs",
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_primitive_collections",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_primitive_collections//jar"],
    runtime_deps = [
      ":org_neo4j_neo4j_unsafe",
      ":org_neo4j_neo4j_common",
      ":org_neo4j_neo4j_resource",
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_resource",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_resource//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_slf4j",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_slf4j//jar"],
    runtime_deps = [
      ":org_neo4j_neo4j_logging",
      ":org_slf4j_slf4j_api",
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_tinkerpop_api",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_tinkerpop_api//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_tinkerpop_api_impl",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_tinkerpop_api_impl//jar"],
    runtime_deps = [
      ":org_neo4j_neo4j_tinkerpop_api",
    ],
  )
  native.java_library(
    name = "org_neo4j_neo4j_unsafe",
    visibility = ["//visibility:public"],
    exports = ["@org_neo4j_neo4j_unsafe//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_objenesis_objenesis",
    visibility = ["//visibility:public"],
    exports = ["@org_objenesis_objenesis//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_scala_lang_modules_scala_parser_combinators_2_11",
    visibility = ["//visibility:public"],
    exports = ["@org_scala_lang_modules_scala_parser_combinators_2_11//jar"],
    runtime_deps = [
      ":org_scala_lang_scala_library",
    ],
  )
  native.java_library(
    name = "org_scala_lang_scala_library",
    visibility = ["//visibility:public"],
    exports = ["@org_scala_lang_scala_library//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_skyscreamer_jsonassert",
    visibility = ["//visibility:public"],
    exports = ["@org_skyscreamer_jsonassert//jar"],
    runtime_deps = [
      ":org_json_json",
      ":commons_collections_commons_collections",
    ],
  )
  native.java_library(
    name = "org_slf4j_jcl_over_slf4j",
    visibility = ["//visibility:public"],
    exports = ["@org_slf4j_jcl_over_slf4j//jar"],
    runtime_deps = [
      ":org_slf4j_slf4j_api",
    ],
  )
  native.java_library(
    name = "org_slf4j_jul_to_slf4j",
    visibility = ["//visibility:public"],
    exports = ["@org_slf4j_jul_to_slf4j//jar"],
    runtime_deps = [
      ":org_slf4j_slf4j_api",
    ],
  )
  native.java_library(
    name = "org_slf4j_log4j_over_slf4j",
    visibility = ["//visibility:public"],
    exports = ["@org_slf4j_log4j_over_slf4j//jar"],
    runtime_deps = [
      ":org_slf4j_slf4j_api",
    ],
  )
  native.java_library(
    name = "org_slf4j_slf4j_api",
    visibility = ["//visibility:public"],
    exports = ["@org_slf4j_slf4j_api//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_twitter4j_twitter4j_core",
    visibility = ["//visibility:public"],
    exports = ["@org_twitter4j_twitter4j_core//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_xmlunit_xmlunit_core",
    visibility = ["//visibility:public"],
    exports = ["@org_xmlunit_xmlunit_core//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "org_xmlunit_xmlunit_matchers",
    visibility = ["//visibility:public"],
    exports = ["@org_xmlunit_xmlunit_matchers//jar"],
    runtime_deps = [
      ":org_xmlunit_xmlunit_core",
      ":org_hamcrest_hamcrest_core",
    ],
  )
  native.java_library(
    name = "org_yaml_snakeyaml",
    visibility = ["//visibility:public"],
    exports = ["@org_yaml_snakeyaml//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "stax_stax_api",
    visibility = ["//visibility:public"],
    exports = ["@stax_stax_api//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "uk_co_datumedge_hamcrest_json",
    visibility = ["//visibility:public"],
    exports = ["@uk_co_datumedge_hamcrest_json//jar"],
    runtime_deps = [
      ":org_skyscreamer_jsonassert",
      ":org_hamcrest_hamcrest_core",
    ],
  )
  native.java_library(
    name = "xerces_xercesImpl",
    visibility = ["//visibility:public"],
    exports = ["@xerces_xercesImpl//jar"],
    runtime_deps = [
      ":xml_apis_xml_apis",
    ],
  )
  native.java_library(
    name = "xml_apis_xml_apis",
    visibility = ["//visibility:public"],
    exports = ["@xml_apis_xml_apis//jar"],
    runtime_deps = [
    ],
  )
  native.java_library(
    name = "xom_xom",
    visibility = ["//visibility:public"],
    exports = ["@xom_xom//jar"],
    runtime_deps = [
    ],
  )
