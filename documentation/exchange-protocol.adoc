...
/.well+known/resourcesync  +-------> | Source description |
              ...
              +---------+---------+    +---------------+
              |                   |    |               |
              |  Other files      |    | Dataset rdf   |
              |  (A.nq, B.nq,...) |    | (actual file) |
              |                   |    |               |
              +-------------------+    +-------+-------+
                                               |
                                       +-------v---------+
...
A capability list may also point to a resource dump and/or changesets, these features are optional and should be provided in addition to the resource list.
...
NOTE: We require now that every dataset to be imported into Timbuctoo have a file (in a supported rdf format) that is
named "Dataset.<rdf extension>". After the dataset is imported into Timbuctoo every change done will
be in a changelist (in nquads-ud format, described below) for this Dataset rdf file.
...
The resources will imported in order of appearance. It is not allowed to later change a resource.
