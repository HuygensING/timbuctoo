...
public class PersonMatcher<T extends Person> extends CompositeMatcher<T> {
  ...
  public static <V extends Person> PersonMatcher<V> likePerson() {
    return new PersonMatcher<V>().ofType((Class<V>) Person.class);
  }

  PersonMatcher<T> ofType(Class<T> type) {
    this.addMatcher(new PropertyEqualtityMatcher<T, Class<T>>("type", type) {

      @SuppressWarnings("unchecked")
      @Override
      protected Class<T> getItemValue(T item) {
  ...
  public static ProjectAPersonMatcher likeProjectAPerson() {
    ProjectAPersonMatcher projectAPersonMatcher = new ProjectAPersonMatcher();
    projectAPersonMatcher.ofType(ProjectAPerson.class);
    return projectAPersonMatcher;
  }
