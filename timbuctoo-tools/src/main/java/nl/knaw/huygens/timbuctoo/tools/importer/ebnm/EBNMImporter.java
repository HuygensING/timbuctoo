...
public class EBNMImporter extends EBNMDefaultImporter {
	...
	public static void main(String[] args) throws Exception {
		...
		// "../../codl_data/data/";
		String importDirName = (args.length > 0)
				? args[0]
				: "../../AtlantischeGids/work/";
	...
	private <T> T readJsonValue(File file, Class<T> valueType) throws Exception {
		...
		if (!converted.equals(text)) {
			...
			handleError("Fixed %d character encoding error%s in '%s'", n,
					(n == 1) ? "" : "s", file.getName());
		}
	...
	public void importAll() throws Exception {
		...
		System.out
				.printf("Number of entries = %d%n", documentatieRefMap.size());
		...
		System.out.printf("Number of entries = %d%n",
				signalementcodeRefMap.size());
	...
	private void removeNonPersistentEnties(StorageManager storageManager,
			IndexManager indexManager) throws IOException, IndexException {
	...
	private void importDocumentatie(Map<String, Reference> referenceMap)
			throws Exception {
		File file = new File(new File(inputDir, DOCUMENTATIE_DIR),
				DOCUMENTATIE_FILE);
		for (XDocumentatie xdocumentatie : readJsonValue(file,
				XDocumentatie[].class)) {
			String jsonId = xdocumentatie._id;
			if (referenceMap.containsKey(jsonId)) {
				handleError("[%s] Duplicate documentatie id %s",
						DOCUMENTATIE_FILE, jsonId);
			} else {
				EBNMDocumentatie documentatie = convert(xdocumentatie);
				String storedId = addDomainEntity(EBNMDocumentatie.class,
						documentatie);
	...
	private EBNMDocumentatie convert(XDocumentatie xdocumentatie) {
		...
		if (type == null) {
			handleError("[%s] Missing type for id %s", DOCUMENTATIE_FILE,
					xdocumentatie._id);
		} else if (type.equals("boek") || type.equals("documentatie")
				|| type.equals("tijdschrift of reeks")
				|| type.equals("artikel in tijdschrift")) {
		...
		documentatie
				.setAnnotatie_bibliografie(xdocumentatie.annotatie_bibliografie);
		...
		documentatie
				.setAnnotatie_exemplaar_alg(xdocumentatie.annotatie_exemplaar_alg);
		...
		documentatie
				.setAnnotatie_rel__gr_kl(xdocumentatie.annotatie_rel__gr_kl);
		...
		documentatie
				.setFysieke_beschrijving(xdocumentatie.fysieke_beschrijving);
		...
		documentatie
				.setTitel_genormaliseerd(xdocumentatie.titel_genormaliseerd);
		...
		documentatie
				.setTrefwoord_oc_artikelen(xdocumentatie.trefwoord_oc_artikelen);
	...
	private void importLexicon(Map<String, Reference> referenceMap)
			throws Exception {
		...
		for (XLexicon xlexicon : readJsonValue(file, XLexicon[].class)) {
			...
			if (referenceMap.containsKey(jsonId)) {
				handleError("[%s] Duplicate lexicon id %s", LEXICON_FILE,
						jsonId);
			} else {
	...
	private EBNMLexicon convert(XLexicon xlexicon) {
		...
		if (type == null) {
			handleError("[%s] Missing type for id %s", LEXICON_FILE,
					xlexicon._id);
		} else if (type.equals("person") || type.equals("tekst")
				|| type.equals("institute") || type.equals("unknown")) {
	...
	private void importTekstdrager(Map<String, Reference> referenceMap)
			throws Exception {
		File file = new File(new File(inputDir, TEKSTDRAGER_DIR),
				TEKSTDRAGER_FILE);
		for (XTekstdrager xtekstdrager : readJsonValue(file,
				XTekstdrager[].class)) {
			String jsonId = xtekstdrager._id;
			if (referenceMap.containsKey(jsonId)) {
				handleError("[%s] Duplicate tekstdrager id %s",
						TEKSTDRAGER_FILE, jsonId);
			} else {
				EBNMTekstdrager tekstdrager = convert(xtekstdrager);
				String storedId = addDomainEntity(EBNMTekstdrager.class,
						tekstdrager);
	...
	private EBNMTekstdrager convert(XTekstdrager xtekstdrager) {
		...
		if (type == null) {
			handleError("[%s] Missing type for id %s", TEKSTDRAGER_FILE,
					xtekstdrager._id);
		...
		tekstdrager
				.setOverige_hs_aanduiding(xtekstdrager.overige_hs_aanduiding);
		...
		tekstdrager
				.setAnnotatie_documentatie(xtekstdrager.annotatie_documentatie);
		...
		tekstdrager
				.setBewaarplaats_en_signatuur(xtekstdrager.bewaarplaats_en_signatuur);
	...
	private void importTekst(Map<String, Reference> referenceMap)
			throws Exception {
	...
	private void importTaal(Map<String, Reference> referenceMap)
			throws Exception {
	...
	private void importPeriode(Map<String, Reference> referenceMap)
			throws Exception {
		...
		for (XPeriode xperiode : readJsonValue(file, XPeriode[].class)) {
			...
			if (referenceMap.containsKey(jsonId)) {
				handleError("[%s] Duplicate periode id %s", PERIODE_FILE,
						jsonId);
			} else {
	...
	private void importRegiocode(Map<String, Reference> referenceMap)
			throws Exception {
		...
		for (XRegiocode xregiocode : readJsonValue(file, XRegiocode[].class)) {
			...
			if (referenceMap.containsKey(jsonId)) {
				handleError("[%s] Duplicate regiocode id %s", REGIOCODE_FILE,
						jsonId);
			} else {
				EBNMRegiocode regiocode = convert(xregiocode);
				String storedId = addDomainEntity(EBNMRegiocode.class,
						regiocode);
	...
	private void importSignalementcode(Map<String, Reference> referenceMap)
			throws Exception {
		File file = new File(new File(inputDir, SIGNALEMENTCODE_DIR),
				SIGNALEMENTCODE_FILE);
		for (XSignalementcode xsignalementcode : readJsonValue(file,
				XSignalementcode[].class)) {
			String jsonId = xsignalementcode._id;
			if (referenceMap.containsKey(jsonId)) {
				handleError("[%s] Duplicate signalementcode id %s",
						SIGNALEMENTCODE_FILE, jsonId);
			} else {
				EBNMSignalementcode signalementcode = convert(xsignalementcode);
				String storedId = addDomainEntity(EBNMSignalementcode.class,
						signalementcode);
	...
	private void importWatermerk(Map<String, Reference> referenceMap)
			throws Exception {
		...
		for (XWatermerk xwatermerk : readJsonValue(file, XWatermerk[].class)) {
			...
			if (referenceMap.containsKey(jsonId)) {
				handleError("[%s] Duplicate watermerk id %s", WATERMERK_FILE,
						jsonId);
			} else {
				EBNMWatermerk watermerk = convert(xwatermerk);
				String storedId = addDomainEntity(EBNMWatermerk.class,
						watermerk);
	...
	public static class XDocumentatie {
		...
		public String toString() {
			return String.format("%-5s %-10s %-40s %-30s %s %s", _id, type,
					onderwerp, regionaam, landcode, titel);
		}
	}
