 ...
 */
public class CopyOfWomenWritersImporter extends WomenWritersDefaultImporter {

  static final Logger LOG = LoggerFactory.getLogger(CopyOfWomenWritersImporter.class);
  ...
  public static void main(String[] args) throws Exception {
    ...
    try {
      ...
      CopyOfWomenWritersImporter importer = new CopyOfWomenWritersImporter(registry, storageManager, relationManager, indexManager, directory);
  ...
  public CopyOfWomenWritersImporter(TypeRegistry registry, StorageManager storageManager, RelationManager relationManager, IndexManager indexManager, String inputDirName) {
  ...
  private void setupRelationMappings() {
    ...
    addRelationMapping("annotated in", "isAnnotatedIn", SAME_ORDER); // reception
    addRelationMapping("authored_by", "isCreatedBy", SAME_ORDER);
    addRelationMapping("biography of", "hasBiography", SAME_ORDER); // reception
    addRelationMapping("censored by", "isCensoredBy", SAME_ORDER);
    addRelationMapping("collaborated_with", "isCollaboratorOf", SAME_ORDER);
    addRelationMapping("comments on", "isWorkCommentedIn", SAME_ORDER); // reception
    addRelationMapping("contains", "containedInAnthology", SAME_ORDER); // reception
    addRelationMapping("copied of", "isCopyOf", SAME_ORDER); // reception
    addRelationMapping("dedicated to", "isDedicatedTo", REVERSED_ORDER); // reception
    addRelationMapping("edition of", "isEditionOf", SAME_ORDER); // reception
    addRelationMapping("granted to", "isPersonAwarded", SAME_ORDER); // reception
    addRelationMapping("has preface", "hasPreface", SAME_ORDER); // reception
    addRelationMapping("intertextual", "isIntertextualOf", SAME_ORDER); // reception
    ...
    addRelationMapping("obituary of", "isObituaryOf", REVERSED_ORDER); // reception
  ...
  private void handleRelation(String line) throws Exception {
    ...
    if (mapping.newName.equals("isTranslationOf") && leftObject.equals("Person")) {
      logToFile("Rejected 'isTranslationOf' (pers-work): %s --> %s", object.leftName, object.rightName);
      return;
    }
    if (mapping.newName.equals("isAdaptationOf") && leftObject.equals("Person")) {
      logToFile("Rejected 'isAdaptationOf' (pers-work): %s --> %s", object.leftName, object.rightName);
      return;
    }
    if (mapping.newName.equals("isAnnotatedIn") && leftObject.equals("Person")) {
      logToFile("Rejected 'isAnnotatedIn' (pers-work): %s --> %s", object.leftName, object.rightName);
      return;
    }
    if (mapping.newName.equals("hasBiography") && leftObject.equals("Document")) {
      logToFile("Rejected 'hasBiography' (work-pers): %s --> %s", object.leftName, object.rightName);
      return;
    }
    if (mapping.newName.equals("containedInAnthology") && leftObject.equals("Person")) {
      logToFile("Rejected 'containedInAnthology' (pers-work): %s --> %s", object.leftName, object.rightName);
      return;
    }
    if (mapping.newName.equals("isCopyOf") && leftObject.equals("Person")) {
      logToFile("Rejected 'isCopyOf' (pers-work): %s --> %s", object.leftName, object.rightName);
      return;
    }
    if (mapping.newName.equals("isCopyOf") && leftObject.equals("Person")) {
      logToFile("Rejected 'isCopyOf' (pers-work): %s --> %s", object.leftName, object.rightName);
    ...
    if (mapping.newName.equals("isDedicatedTo") && leftObject.equals("Document")) {
      logToFile("Rejected 'isDedicatedTo' (work-work): %s --> %s", object.leftName, object.rightName);
      return;
    }
    if (mapping.newName.equals("hasPreface") && leftObject.equals("Person")) {
      logToFile("Rejected 'hasPreface' (pers-work): %s --> %s", object.leftName, object.rightName);
      return;
    }
    if (mapping.newName.equals("isIntertextualOf") && leftObject.equals("Person")) {
      logToFile("Rejected 'isIntertextualOf' (pers-work): %s --> %s", object.leftName, object.rightName);
      return;
    }
    if (mapping.newName.equals("isObituaryOf") && leftObject.equals("Document")) {
      logToFile("Rejected 'isObituaryOf' (work-work): %s --> %s", object.leftName, object.rightName);
      return;
    }

    if (mapping.newName.equals("isWorkCommentedIn") && leftObject.equals("Person")) {
      relationRef = relationTypes.get("isPersonCommentedIn");
      Preconditions.checkState(relationRef != null, "Missing relation type 'isPersonCommentedIn'");
    }
    if (mapping.newName.equals("isPersonAwarded") && leftObject.equals("Document")) {
      relationRef = relationTypes.get("isWorkAwarded");
      Preconditions.checkState(relationRef != null, "Missing relation type 'isPersonCommentedIn'");
    }
