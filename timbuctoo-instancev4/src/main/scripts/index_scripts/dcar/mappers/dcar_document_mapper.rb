...
class DcarDocumentMapper < DefaultMapper
  ...
  def add_person_receptions(record)
    unless record['@relations'].nil?
      DcarDocumentMapper.person_reception_names.each do |rec_rel|
  ...
  def add_document_receptions(record)
    unless record['@relations'].nil?
      DcarDocumentMapper.document_reception_names.each do |rec_rel|
        unless record['@relations'][rec_rel].nil?
          record['@relations'][rec_rel].each do |rr_data|
            wanted_reception = Hash.new
            wanted_reception[:reception_id] = record['_id']
  ...
  def DcarDocumentMapper.person_reception_names
    [
        "isBiographyOf",
        "commentsOnPerson",
        "isDedicatedTo",
  ...
  def DcarDocumentMapper.document_reception_names
    [
        "isEditionOf",
        "isSequelOf",
        "isTranslationOf",
