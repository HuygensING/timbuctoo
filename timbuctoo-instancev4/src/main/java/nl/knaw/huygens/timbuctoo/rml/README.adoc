...
Joining::
Rml is based on https://www.w3.org/TR/r2rml/[R2RML].
R2RML is very much dependent on the database engine to make int useful.
This is apparent when you read the algorithm for implementing ReferencingObjectMaps which very much depends on being able to do a SQL join.
To implement this in RML we have added a method to datasources called `willBeJoinedOn`.
A dataSource is supposed to store this value in a HashMap or something and then merge that value into it's result set when getRows is called.
+
The code to implement this does not yet support circular dependencies very well and does not support self-joins at all.
Mapping columns::
...
R2RML does not have this support.
