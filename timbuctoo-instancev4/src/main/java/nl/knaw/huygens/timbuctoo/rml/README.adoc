...
In order for this library to support big datasets without the leverage (which SQL based databases provide), data rows are expected to be streamed.

This however introduces the problem of handling tables which refer to eachother (possibly with cyclic dependency) as explained in the next sub-sections.
