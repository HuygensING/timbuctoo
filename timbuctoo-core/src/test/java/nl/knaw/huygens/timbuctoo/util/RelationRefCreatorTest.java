...
public class RelationRefCreatorTest {
  ...
  private Storage setupStorage() throws StorageException {
    ...
    when(storage.getItem(TARGET_TYPE, TARGET_ID)).thenReturn(targetEntity());
    when(storage.getItem(SOURCE_TYPE, SOURCE_ID)).thenReturn(sourceEntity());
    return storage;
  }
  ...
  public void createRegularThrowsAStorageExceptionWhenTheStorageDoes() throws Exception {
    // setup
    when(storage.getItem(TARGET_TYPE, TARGET_ID)).thenThrow(new StorageException());
  ...
  public void createInverseThrowsAStorageExceptionWhenTheStorageDoes() throws Exception {
    // setup
    when(storage.getItem(SOURCE_TYPE, SOURCE_ID)).thenThrow(new StorageException());
